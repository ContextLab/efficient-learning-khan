...
To begin to test this smoothness assumption, we sought
...
diverse concepts, courses, and students. This requires that the conceptual content of
interest be discovered \textit{automatically}, rather than relying on
...
\begin{figure}[tp]
    ...
    completing three 13-question multiple-choice quizzes and watching two Khan Academy
...
\begin{figure}[tp]
    ...
    \textit{trajectories}.} After training the model on the sliding windows from
...
\begin{figure}[tp]
    ...
    topics may be found in Supplementary Table~\topics. \textbf{B. Relationships between topic
...
this approach to the questions from each quiz taken throughout the experiment, separately. 
From just a few questions per quiz (see~\nameref{subsec:traces}), we obtain a
...
estimated knowledge (from the third quiz; Fig.~\ref{fig:predictions}, right
panel) for \textit{all} questions exhibited a positive shift. However, the
estimated knowledge for held-out correctly answered questions remained
greater than that for held-out incorrectly answered questions ($t(628) = 2.045,~p
= 0.041$). This third contrast reflects a ceiling effect: when knowledge is
...
that our knowledge prediction framework is most informative when participants
exhibit variability in their knowledge of the content captured by the text
embedding model.
...
that contained in the two lectures.
...
\begin{figure}[tp]
    ...
    the same general format as the knowledge maps in Panel A, but here the
    shading at each coordinate indicates the \textit{difference} between the
    corresponding coordinates in the indicated \textit{pair} of knowledge
    ...
    Word clouds for sampled points in topic space.} Each word cloud displays the
    weighted blend of words underlying the topic proportions represented at the
    corresponding colored star's location on the maps. In each word cloud, the
    words' relative sizes correspond to their relative weights at the starred
    location, and their colors indicate their relative weights in the
...
watched prior to taking Quiz~2. This localization is non-trivial: these knowledge
...
Fundamental Forces} embedding tended to be weighted more heavily by the topics
...
more heavily by the topics expressed in \textit{that} lecture. And the
...
knowledge of those concepts changes with training (Fig.~\ref{fig:knowledge-maps}).

Over the past several years, the global pandemic has forced many educators to
suddenly adapt to teaching remotely~\citep{MoseEtal21, ShimLee20, KawaEtal21, Whal20}. This change
in world circumstances is happening alongside (and perhaps accelerating)
geometric growth in the availability of high-quality online courses from
...
shared and viewed by increasingly large segments of the world's population. This
...
For example, we can ask: what defines an effective course or training program?
Which aspects of teaching might be optimized and/or augmented by automated tools? How and why do
learning needs and goals vary across people? How might we lower barriers of access
...
Participants received optional course credit for enrolling. We asked each participant to
...
all participants, a range of alertness levels were reported (range:
-2--1; mean: -0.10; standard deviation: 0.84).
...
(see Fig.~\ref{fig:experiment}). The questions appearing on each quiz, for each
participant, were randomly chosen from the full set of 39, with the constraints
that (a) each quiz contain exactly 5 questions about Lecture~1, 5 questions about
...
discover a set of (up to) $k$ ``topics'' or ``themes.'' Formally, each topic is defined
as a distribution of weights over each word in the model's vocabulary (i.e., the union
...
were assigned uniform weights) after training. This indicated that the number of topics was
...
transcripts (which, in prior work, we have found to be of sufficiently near-human
quality to yield reliable data in behavioral studies;~\citealp{ZimaEtal18}) when
...
timestamp corresponding to the midpoint between the timestamps for its first and last lines.
These sliding windows ramped up and down in length at the
beginning and end of each transcript, respectively. In other words, each transcript's first
...
whose elements are always non-negative and sum to one. Although UMAP is an invertible
...
which are incompatible with the topic modeling framework. To protect against
...
vector of non-negative values, and normalized them to sum to one.
