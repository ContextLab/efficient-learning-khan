...
\newcommand{\ldaVsBERT}{8}
\newcommand{\jaccard}{9}   % word match figure
...
(e.g.,~\citealp{LandDuma97, DeerEtal90, BleiEtal03, BleiLaff06, MikoEtal13a,
CerEtal18, BrowEtal20, ViswEtal17}) also attempt to capture the deeper meaning
\textit{underlying} those atomic elements. These models consider not only the
co-occurrences of those elements within and across documents, but (in many
cases) also patterns in how those elements appear across different scales
...
in those documents have very little overlap}. In this way, ``concepts'' are
defined implicitly by the model's geometry~\citep[e.g., how the embedding coordinate
...
person's knowledge take (speculatively)? First, we might require a means of
...
It is important to clarify that although we use topic model-derived embeddings
to \textit{characterize} the conceptual content of the lectures and questions,
we do not claim that the topic model itself \textit{understands} the conceptual
content of the lectures or questions. Rather, we view the topic model as a tool
for capturing the \textit{structure} of the conceptual content of the lectures
...
After watching \textit{Four Fundamental Forces}, estimated knowledge for
held-out correctly answered questions (from the second quiz;
Fig.~\ref{fig:predictions}, middle column) exhibited a positive shift relative
to held-out incorrectly answered questions. This held when we included all
questions in the analysis ($U = 58332, p < 0.001$), when we predicted knowledge
across-lectures (\textit{Four Fundamental Forces}: $U = 6749.5, p = 0.014$;
\textit{Birth of Stars}: $U = 8480, p = 0.016$), and when we predicted
knowledge at the embedding coordinates of held-out \textit{Four Fundamental
Forces} questions using other \textit{Four Fundamental Forces} questions from
the same quiz and participant ($U = 7224, p < 0.001$). This difference did
\textit{not} hold for within-lecture predictions of \textit{Birth of Stars}
knowledge ($U = 7419, p = 0.739$). Again, we suggest that this might reflect a
floor effect whereby knowledge about the content of the \textit{Birth of Stars}
material is relatively low everywhere in that region of text embedding space.

Finally, after watching \textit{Birth of Stars}, estimated knowledge for
held-out correctly answered questions (from the third quiz;
Fig.~\ref{fig:predictions}, right column) was higher for held-out correctly
...
we carried out across-lecture predictions (\textit{Four Fundamental Forces}: $U
= 6684.5, p = 0.032$; \textit{Birth of Stars}: $U = 6414.5, p = 0.002$), and
and when we carried out within-lecture predictions of held-out \textit{Birth of
Stars} questions using other \textit{Birth of Stars} questions from the same
quiz and participant ($U = 6126, p = 0.006$). However, we found the
\textit{opposite} effect when we carried out within-lecture predictions of
held-out \textit{Four Fundamental Forces} questions using other \textit{Four
Fundamental Forces} questions from the same quiz and participant ($U = 6734, p
= 0.027$). Specifically, held-out correctly answered Quiz 3 questions about
\textit{Four Fundamental Forces} had reliably \textit{lower} estimated
knowledge than held-out incorrectly answered questions. Speculatively, we
suggest that this may reflect participants forgetting some of the \textit{Four
Fundamental Forces} content. If this forgetting happens in a relatively
``random'' way (with respect to spatial distance within the text embedding
space), then it could explain why some held-out questions about \textit{Four
Fundamental Forces} were answered incorrectly, even if questions at nearby
coordinates (i.e., about similar content) were answered correctly. This might
lead our approach to over-estimate knowledge for held-out questions about
``forgotten'' knowledge that particpants answered incorrectly.

That the knowledge estimates derived from the text embedding space reliably
distinguish between held-out correctly versus incorrectly answered questions
(Fig.~\ref{fig:predictions}) suggests that the text embedding space bears at
...
answered that same question \textit{in}correctly. Again, as we move away from
$X$ in the embedding space, how does our confidence that the participant does
\textit{not} know about the content change with distance? We reasoned that,
...
undesireable behavior in the UMAP embedding procedure~\citep{McInEtal18},
...
suggests that participants also \textit{conceptualize} similarly the content
...
given domain (e.g., the domain of physics lectures and questions) \textit{and}
sufficiently broad so as to enable them to cover a wide range of domains. For
example, we found that the embeddings derived even from much larger and more
modern models like BERT~\citep{DevlEtal18}, GPT~\citep{ViswEtal17},
LLaMa~\citep{TouvEtal23}, and others that are trained on enormous text corpora,
...
Fig.~\ref{fig:sliding-windows}C, the LDA embeddings for \textit{Four
Fundamental Forces} questions (blue dots) appear closer to the \textit{Four
Fundamental Forces} lecture trajectory (blue line), whereas the LDA embeddings
for \textit{Birth of Stars} questions (green dots) appear closer to the
\textit{Birth of Stars} lecture trajectory (green line). The BERT embeddings of
...
\textit{generate} text, combined with a smaller model that can \textit{infer}
what learners know and how their knowledge changes over time.  Such a hybrid
...
\textit{topics} are discussed (e.g., when words like ``cloud,'' ``atom,''
``dense,'' etc., are mentioned \textit{together}). The Jaccard similarity-based
matches, on the other hand, are strong when \textit{any} of these words are
...
that are semantically related, as implied by their co-occurring in the same documents. After
...
augmenting the Natural Language Toolkit~\citep[NLTK; ]{BirdEtal09} English stop
word list with the following additional words, selected using the stop word
approach suggested by~\citep{BoydMimn14}: ``actual,'' ``actually,'' ``also,''
