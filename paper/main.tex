...
Next, we sought to characterize what information participants' performance on
each of the three quizzes could provide about their conceptual knowledge at
that point in time. Traditional approaches to evaluating students'
performance on short-form knowledge assessments, such as those in our study,
entail computing the proportion of correctly answered questions to assign a
simple numeric score. While this may afford some measure of the
\textit{extent} of a learner's knowledge, such a score provides little
information to either the teacher or the learner about the particular
\textit{contents} of their knowledge---in other words, raw
``proportion-correct'' measures may capture \textit{how much} a student
knows, but not \textit{what} they know. For instance, suppose a participant
in our study was highly knowledgeable about fundamental physical forces (i.e
., lecture 1 content) but unfamiliar with how stars are formed (i.e., lecture
2 content), while a second participant was unfamiliar with fundamental forces
but had extensive prior knowledge about star formation. Since quiz 1
(completed before viewing either lecture) contained an equal number of
questions about each lecture's content (see Fig.~\ref{fig:experiment}), these
two participants could easily achieve the same proportion-correct score
despite their conceptual knowledge differing substantially. How might we
distinguish these individuals?

We hypothesized that our text embedding model, which we had trained on
transcripts of the two lectures to characterize their conceptual contents,
should also allow us to capture the conceptual knowledge probed by each quiz
question. If our model successfully represented information about the deeper
conceptual content of the lectures (i.e., beyond surface-level details such
as particular word choices) then we should be able to recover a
correspondence between their embeddings and embeddings of other text that
reflects related concepts (e.g., quiz questions that were ostensibly, by
design, ``about'' one of the two lecture) despite differences in exact
verbiage. Intuitively, while the content of each given lecture will naturally
vary from moment to moment, the particular combination of dimensions along
which its trajectory varies across moments correspond to topics that encode
meaningful ``information''~\citep{Fish22}  about that lecture's dynamic
conceptual content. We therefore expected that quiz questions pertaining to
those same concepts might express the same set of topics.
...
weight variability}. The bar plots display the variance of each topic's weight 
across lecture timepoints (top row) and questions (bottom row); colors 
denote topics. The top-weighted words from the most ``expressive'' (i.e., 
variable across observations) topic from each lecture are displayed in the 
upper right (orange: topic 2; yellow-green: topic 4). The top-weighted words 
from the full set of topics may be found in Table~\topics. \textbf{B. Relationships between topic weight variability.} Pairwise correlations between the distributions of topic weight variance for each lecture and question category. Each row/column corresponds to a bar plot in panel A.}

    \label{fig:topics}
\end{figure}

We used our model to transform the text of each question in our pool into
the same embedding space as the two lectures' trajectories 
(Fig.~\ref{fig:sliding-windows}C). This yielded a single 15-dimensional
coordinate (i.e., topic vector) for each question. We then computed the variance
of each topic's weight across timepoints of each lecture, and across 
questions from each category (i.e., lecture 1-related, lecture 2-related, and
general physics knowledge; Fig.~\ref{fig:topics}A). Visual inspection of 
Figure~\ref{fig:topics}A suggests a strong correspondence between the sets of 
topics expressed by each lecture and its related questions, with only limited 
overlap in the topics expressed by non-matched lecture-question set pairs. 
To quantify these apparent similarities and differences, we computed the correlation 
between each pair of topic-weight variance distributions (Fig.~\ref{fig:topics}B). 
We found that our model represented the contents of lecture-related questions 
using a combination of topic dimensions (and relative variation along those topic dimensions) that was highly similar to that of 
their reference lecture (\textit{Four Fundamental Forces} (\textit{FFF}) 
questions vs.~lecture: Pearson's $r(13) = 
0.824,~p<0.001,~95\%~\mathrm{confidence~interval~(CI)} = [0.696, 0.973]$; 
\textit{Birth of Stars} (\textit{BoS}) questions vs.~lecture: $r(13) = 
0.801,~p<0.001,~95\%~\mathrm{CI} = [0.539, 0.958]$) but diverged significantly 
from that of the non-reference lecture (\textit{FFF} questions vs.~\textit{BoS} 
lecture: $r(13) = -0.620,~p = 0.014,~95\%~\mathrm{CI} = [-0.871, -0.326]$; 
\textit{BoS} questions vs.~\textit{FFF} lecture: $r(13) = -0.550,~p = 
0.034,~95\%~\mathrm{CI} = [-0.803, -0.246]$). This indicated that our model
captured the conceptual contents of the lectures and quiz questions 
sufficiently well to differentiate between questions relating to one lecture 
versus the other.

%This indicated that our model
%captures the conceptual contents of the lectures and quiz questions 
%sufficiently well to differentiate between questions relating to one lecture 
%versus the other, and could potentially be used to assess participants' knowledge for each lecture 



% , and could be used to assess participants' knowledge about
%each lecture independently, based on the \textit{specific} questions they
%answered correctly or incorrectly on each quiz. 
%providing a slightly more 
%nuanced measure of what concepts they do and do not know than a simple 
%proportion-correct score.

%When we compared the
%variability in topic weights across each lecture's timepoints, and across
%questions from each category (i.e., lecture 1-related, lecture 2-related, and
%general physics knowledge), we found multiple interesting patterns (Fig.~\ref{fig:topics}). First,
%there is a strong correspondence between the sets of topics expressed by each
%lecture and its relevant questions. This indicates that our model is
%sensitive to the underlying conceptual content tested by different question,
%and should allow us to differentially assess participants' knowledge for the
%two lectures based on the \textit{specific} questions they answered correctly
%or incorrectly on each quiz. Second, the topics expressed by the general
%physics knowledge questions comprise a nonspecific mix of topics from the two
%lectures (4 from lecture 1; 5 from lecture 2), most of which are expressed to
%a relatively low degree. This indicates that our model should enable us to
%discriminate questions that are highly relevant to our assessment of
%participants' knowledge about concepts from the lectures from those that are
%not (or potentially less so). Third, the two lectures' trajectories vary
%along largely non-overlapping topic dimensions (e.g., variations in weight
%for topics 2, 3, 8, 12, 13, and 15 are specific to lecture 1; variations in
%weight for topics 1, 4, 5, 6, 7, and 10 are specific to lecture 2), but also
%share a small number of expressed topics in common (topics 11 and 14). This
%indicates that our lecture is able to simultaneously capture 

This could enable us to estimate participants' knowledge for each of the two lectures, separately, by 

This aspect of our model could enable us to assess participants' knowledge for each lecture separately, based on which \textit{specific} questions they answered right on wrong on each quiz, providing a slightly more refined estimate of the concepts they do and do not know than single proportion-correct score. However, while lectures are often organized around a single, broad theme at a coarse timescale, they typically cover...
%While individual lectures may be organized around a single broad theme at a coarse scale, 


\textcolor{red}{\textbf{Paxton stopped here}}
...
\begin{figure}[tp]
    ...
    correlation between the indicated question and the lecture trajectory. The
    associated questions' text and snippets of the lectures' transcripts from the surrounding 30 seconds, are displayed at the bottom of the
    figure.}

    \label{fig:question-correlations}
\end{figure}

Although an individual lecture may be organized around a single broad theme at a
...
Briefly, our approach uses a topic model (Latent Dirichlet Allocation;~\citealp{BleiEtal03}), trained on a set
...
We fetched these automated transcripts using the \texttt{youtube-transcript-api} 
Python package (\textcolor{red}{\textbf{Jeremy can you add a citation for this?} 
https://github.com/jdepoix/youtube-transcript-api}). The transcripts consisted of 
one timestamped line of text for every few seconds (mean: 2.34s; standard 
deviation: 0.83s) of spoken content in the video (i.e., corresponding to each 
individual caption that would appear on-screen if viewing the lecture via YouTube, 
and when those lines would appear). We
defined a sliding window length of (up to) $w = 30$ transcript lines, and
assigned each window a timestamp corresponding to the midpoint between its first
...
windows. After performing various standard text preprocessing (e.g., normalizing case, 
lemmatizing, removing punctuation and stop-words), we treated the text from each 
sliding window as a single ``document,''
...
Euclidean \textcolor{red}{should we say something like cosine instead of euclidean?} distance, correlation, etc.) topic vectors. In general, the
...
resulting timeseries to one vector per second. We also used the fitted model to obtain topic
...
timepoints and questions across both lectures and all five question used to estimate the knowledge trace \textcolor{red}{\textbf{Note: make sure this is correct in results section; not sure i caught all instances}}. We
...
vectors, $L$, and $i$ and $j$ index the topic vectors of questions used to estimate the knowledge trace, $Q$. Note that ``correct'' denotes the set of indices of
...
and~\individualLearningMapsB), we used Uniform Manifold Approximation and Projection (UMAP; \citealp{McInEtal18a}) to construct a 2D
...
learning, i.e., changes in knowledge) at each of the resulting 10,000
coordinates.
