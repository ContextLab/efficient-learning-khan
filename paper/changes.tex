...
%DIF ADD main.tex   Tue Oct 24 18:58:36 2023
...
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Predicting knowledge at the embedding coordinates of
    held-out questions.}} \DIFaddendFL Separately for each quiz (panel), we plot the
    ...
    of \DIFdelbeginFL \DIFdelFL{estimated
    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{predicted }\DIFaddendFL knowledge at the coordinates of correctly \DIFdelbeginFL \DIFdelFL{versus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL incorrectly
...
greater than that }\DIFdelend \DIFaddbegin \DIFadd{about }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{were reliably
...
This might lead our approach to over-estimate knowledge }\DIFaddend for held-out \DIFdelbegin \DIFdel{incorrectly }\DIFdelend \DIFaddbegin \DIFadd{questions
...
= 0.041$) }\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{fig:predictions}) suggests that spatial relationships in the text
embedding space can help explain what participants know. But how far does this
explanatory power extend? For example, suppose we know that a participant
answers a question (at embedding coordinate $x$) correctly. As we move away
from $x$ in the embedding space, how does their knowledge ``fall off'' with
distance? Or, suppose the participant instead answered that same question
}\textit{\DIFadd{in}}\DIFadd{correctly. Again, as we move away from $x$ in the embedding space,
how do the chances that the participant does }\textit{\DIFadd{not}} \DIFadd{know about the
content change with distance? We reasoned that, assuming our space is capturing
something about how participants actually organize their knowledge, conceptual
knowledge right around $x$ should be similar to the participant's knowledge of
the content at $x$. And at another extreme, at some distance (after moving
sufficiently far away from $x$), our guesses about what participants know
(based on their response to the question at location $x$) should be no better
than guessing based on their overall proportion of correctly answered
questions—i.e., if $y$ is very far away from $x$, all we can do with the
participant's response to $x$ is guess that ``their performance on quiz
questions about $y$ will be about equal to their average performance on quiz
...
\DIFdelbegin \DIFdel{This third contrast reflects a ceiling effect: when knowledge is
...
undesirable behavior in the UMAP embedding procedure~\mbox{%DIFAUXCMD
\citep{McInEtal18a}}\hskip0pt%DIFAUXCMD
,
...
suggested by~\mbox{%DIFAUXCMD
\citep{BoydMimn14}}\hskip0pt%DIFAUXCMD
: ``actual,'' ``actually,'' ``also,'' ``bit,''
...
words, and lasting for an average of 62.22~seconds. We }\DIFaddend treated the text from
