...
For each \DIFdelbegin \DIFdel{question}\DIFdelend \DIFaddbegin \DIFadd{quiz question a participant answered}\DIFaddend , in turn, we used Equation~\ref{eqn:prop} to \DIFdelbegin \DIFdel{predict each participant's }\DIFdelend \DIFaddbegin \DIFadd{estimate their }\DIFaddend knowledge at the given question's embedding space coordinate \DIFdelbegin \DIFdel{, using all }\textit{\DIFdel{other}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{based on other }\DIFaddend questions that participant answered on the same quiz. 
\DIFdelbegin \DIFdel{For each }\DIFdelend \DIFaddbegin \DIFadd{We repeated this for all participants, and for each of the three quizzes. 
Then, separately for each }\DIFaddend quiz, we \DIFdelbegin \DIFdel{grouped these
predicted knowledge values into two distributions: one for the predicted knowledge at the
coordinates of }\textit{\DIFdel{correctly}} %DIFAUXCMD
...
\DIFdel{answered
questions (Fig.~\ref{fig:predictions}).
We then used Mann-Whitney $\U$-tests to
compare the means of these distributions of predicted knowledge }\DIFdelend \DIFaddbegin \DIFadd{fit a generalized linear mixed model (GLMM) with a logistic link function to explain the likelihood of correctly answering a question as a function of estimated knowledge for its embedding coordinate, while accounting for random variation among participants and questions (see }\nameref{subsec:glmm}\DIFadd{).
To assess the predictive value of the knowledge estimates, we compared each GLMMs to an analogous (i.e., nested) ``null'' model that did not consider estimated knowledge using parametric bootstrap likelihood-ratio tests}\DIFaddend .
...
\begin{figure}[tp]
    ...
    \DIFaddbeginFL \caption{\textbf{\DIFaddFL{Predicting success on held-out questions using estimated knowledge.}}
    \DIFaddFL{We used generalized linear mixed models (GLMMs) to model the likelihood of correctly answering a quiz question as a function of estimated knowledge for its embedding coordinate (see }\nameref{subsec:glmm}\DIFaddFL{). 
    Separately for each quiz (column), we examined this relationship based on three different sets of knowledge estimates: knowledge for each question based on all other questions the same participant answered on the same quiz (``All questions''; top row), knowledge for each question about one lecture based on all questions (from the same participant and quiz) about the }\textit{\DIFaddFL{other}} \DIFaddFL{lecture (``Across-lecture''; middle rows), and knowledge for each question about one lecture based on all other questions (from the same participant and quiz) about the }\textit{\DIFaddFL{same}} \DIFaddFL{lecture (``Within-lecture''; bottom rows). 
...
{%DIFAUXCMD
\textbf{\DIFdelFL{Predicting knowledge at the embedding coordinates of
    held-out questions.}} %DIFAUXCMD
\DIFdelFL{Separately for each quiz (column), we plot the
    ...
    coordinate of a held-out question about the }\textit{\DIFdelFL{other}} %DIFAUXCMD
\DIFdelFL{lecture. In the
    ...
    question about the }\textit{\DIFdelFL{same}} %DIFAUXCMD
\DIFdelFL{lecture. We repeated each of these
...
%DIFDELCMD <     %%%
\DIFdelendFL \label{fig:predictions}
\end{figure}

We carried out \DIFdelbegin \DIFdel{these analyses in three different ways. First, we used all (but one) of the questions from a given quiz (and participant) to predict knowledge at the
embedding coordinate of a held-out question }\DIFdelend \DIFaddbegin \DIFadd{three different versions of the analyses described above, wherein we considered different sources of information in our estimates of participants' knowledge for each quiz question. 
First, we estimated knowledge at each question's embedding coordinate using }\textit{\DIFadd{all other}} \DIFadd{questions answered by the same participant on the same quiz }\DIFaddend (``All questions''\DIFdelbegin \DIFdel{in
}\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend Fig.~\ref{fig:predictions}\DIFaddbegin \DIFadd{, top row}\DIFaddend ).
This test was intended to \DIFdelbegin \DIFdel{serve as an overall
baseline for the }\DIFdelend \DIFaddbegin \DIFadd{assess the overall }\DIFaddend predictive power of our approach. 
...
If }\DIFaddend we held out \DIFdelbegin \DIFdel{individual questions and predicted their knowledge at the held-out questions '
embedding coordinates, we found no reliable differences in the predictions when
the }\DIFdelend \DIFaddbegin \DIFadd{a single correctly answered question and computed the proportion of remaining questions answered correctly, that proportion would be $(n - 1) / 12$. 
Whereas if we held out a single }\textit{\DIFadd{incorrectly}} \DIFadd{answered question and did the same, that proportion would be $n / 12$. 
Thus for a given participant and quiz, a ``knowledge estimate'' computed as the simple (i.e., unweighted) remaining proportion-correct is perfectly inversely related to success on a }\DIFaddend held-out question\DIFdelbegin \DIFdel{had been correctly versus incorrectly answered . This
``null'' effect persisted when we used }\textit{\DIFdel{all}} %DIFAUXCMD
\DIFdel{of the Quiz 1 questions
from a given participant to predict a }\DIFdelend \DIFaddbegin \DIFadd{: it will always be }\textit{\DIFadd{lower}} \DIFadd{for correctly answered questions than for incorrectly answered questions. 
...
Specifically, higher estimated knowledge }\DIFaddend at the embedding coordinate of a held-out \DIFdelbegin \DIFdel{question about the }\textit{\DIFdel{other}} %DIFAUXCMD
\DIFdel{lecture (``Across-lecture''; predicting knowledge for held-out
}\textit{\DIFdel{Four Fundamental Forces Questions}} %DIFAUXCMD
...
\DIFdel{questions: $\U = 8202.5,~p = 0.161$)}\DIFdelend \DIFaddbegin \DIFadd{Quiz~1 question was associated with a lower likelihood of answering the question correctly (odds ratio $(OR) = 0.136$}\DIFaddend , \DIFdelbegin \DIFdel{and when we used
...
Stars}}%DIFAUXCMD
\DIFdel{: $\U = 8125,~p = 0.204$). 
We believe that this reflects a floor effect:
...
Fig.
~\ref{fig:predictions}, middle column) exhibited a significant positive
...
we predicted knowledge at }\DIFdelend \DIFaddbegin \DIFadd{lecture. 
Specifically, for questions that participants answered on Quiz~1, prior to watching either lecture, knowledge for }\DIFaddend the embedding coordinates of \DIFdelbegin \DIFdel{held-out }\DIFdelend \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{questions }\DIFdelend \DIFaddbegin \DIFadd{-related questions estimated }\DIFaddend using other \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{questions from the same quiz and participant ($\U = 7224,~p < 0.001$). 
This
difference did }\textit{\DIFdel{not}} %DIFAUXCMD
\DIFdel{hold for within-lecture knowledge predictions at
knowledge at embedding space coordinates of }\DIFdelend \DIFaddbegin \DIFadd{-related questions did not reliably predict whether those questions were answered correctly ($OR = 1.891,\ \lambda_{LR} = 2.293,\ 95\%\ \textnormal{CI} = [2.091,\ 2.622],\ p = 0.139$). 
The same was true of knowledge estimates for }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{questions
($\U = 7419,~p = 0.739$). 
Again, we suggest that this might reflect a floor
effect whereby, at that point in the participants' training, their knowledge about the content of the }\DIFdelend \DIFaddbegin \DIFadd{-related questions based on other }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{material is relatively low
everywhere in that region of text embedding space.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Finally, after watching }\DIFdelend \DIFaddbegin \DIFadd{-related questions ($OR = 0.722,\ \lambda_{LR} = 5.115,\ 95\%\ \textnormal{CI} = [0.094,\ 0.146],\ p = 0.738$). 
When we recomputed these within-lecture knowledge estimates using questions from Quiz~2---which participants took immediately after viewing }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{but prior to viewing }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{, predicted knowledge for
...
predictions (}\DIFdelend \DIFaddbegin \DIFadd{---we found that they now reliably predicted success on }\DIFaddend \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{: $\U = 6684.5,~p = 0.032$;
}\DIFdelend \DIFaddbegin \DIFadd{-related questions ($OR = 9.023,\ \lambda_{LR} = 18.707,\ 95\%\ \textnormal{CI} = [10.877,\ 22.222],\ p = 0.001$) but not on }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{: $\U = 6414.5,~p = 0.002$), and and when we carried out
}\DIFdelend \DIFaddbegin \DIFadd{-related questions ($OR = 0.306,\ \lambda_{LR} = 5.115,\ 95\%\ \textnormal{CI} = [4.624,\ 5.655],\ p = 0.055$).
Using participants' responses from Quiz~3 (taken immediately after viewing }\textit{\DIFadd{Birth of Stars}}\DIFadd{), we found that }\DIFaddend within-lecture knowledge \DIFdelbegin \DIFdel{predictions for held-out }\DIFdelend \DIFaddbegin \DIFadd{estimates for }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{questions using other }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{questions from the same quiz and
participant ($\U = 6126,~p = 0.006$}\DIFdelend \DIFaddbegin \DIFadd{-related questions could now reliably predict success on those questions ($OR = 5.467,\ \lambda_{LR} = 10.670,\ 95\%\ \textnormal{CI} = [7.998, 12.532],\ p = 0.006$}\DIFaddend ). 
...
}\DIFdelend \DIFaddbegin \DIFadd{estimates for }\DIFaddend \textit{Four Fundamental Forces} questions \DIFdelbegin \DIFdel{using other }\textit{\DIFdel{Four Fundamental
Forces}} %DIFAUXCMD
\DIFdel{questions from the same quiz and participant ($\U = 6734,~p = 0.027$).
Specifically, on Quiz }\DIFdelend \DIFaddbegin \DIFadd{answered on Quiz~}\DIFaddend 3 \DIFdelbegin \DIFdel{, our knowledge predictions for held-out correctly
...
}\DIFdelend \DIFaddbegin \DIFadd{were no longer directly related to the likelihood of successfully answering them and instead exhibited the inverse relationship we would expect to arise from unstructured knowledge (with respect to the embedding space; $OR = 0.013,\ \lambda_{LR} = 14.648,\ 95\%\ \textnormal{CI} = [10.695, 23.096],\ p = 0.001$).
}\DIFaddend Speculatively, we suggest that this may reflect participants forgetting some of the \textit{Four Fundamental Forces} content \DIFaddbegin \DIFadd{(e.g., perhaps in favor of prioritizing encoding the just-watched }\textit{\DIFadd{Birth of Stars}} \DIFadd{content in preparation for the third quiz)}\DIFaddend . 
If this forgetting happens in a relatively ``random'' way (with respect to spatial distance within the \DIFdelbegin \DIFdel{text
}\DIFdelend embedding space), then it could explain why some held-out questions about \textit{Four Fundamental Forces} were answered incorrectly, even if questions at nearby coordinates (i.e., about similar content) were answered correctly.
