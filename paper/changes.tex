...
%DIF ADD main.tex   Tue Feb 20 13:03:38 2024
%DIF 2a2
...
\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{other}} \DIFaddendFL lecture \DIFaddbeginFL \DIFaddFL{(``Across-lecture''; bottom rows)}\DIFaddendFL .
...
unstructured (Fig.~\ref{fig:predictions},
left column). When we
held out
...
correctly answered remaining questions is always }\textit{\DIFadd{lower}} \DIFadd{when the
}\DIFaddend held-out \DIFdelbegin \DIFdel{question had been correctly versus incorrectly answered. This
...
knowledge at the embedding coordinate of a }\DIFdelend \DIFaddbegin \DIFadd{question was answered correctly than when it was answered incorrectly.
...
embedding-space distance from the }\DIFaddend held-out \DIFdelbegin \DIFdel{question about the }\textit{\DIFdel{other}} %DIFAUXCMD
\DIFdel{lecture (``Across-lecture''; predicting knowledge
for }\DIFdelend \DIFaddbegin \DIFadd{question; see Eqn.~\ref{eqn:prop}),
...
knowledge about the questions' content, then we would expect }\textit{\DIFadd{higher}}
\DIFadd{estimated knowledge for }\DIFaddend held-out \DIFdelbegin \textit{\DIFdel{Four Fundamental Forces Questions}} %DIFAUXCMD
...
from the }\textit{\DIFadd{other}} \DIFadd{lecture. Here we again observed a similar pattern of
...
about \DIFdelbegin \DIFdel{more subtly different contentwithin the same lecture}\DIFdelend \DIFaddbegin \DIFadd{similar content, and generalize across content areas, provided that
...
predictive of their performance}\DIFaddend .
...
\DIFadd{As necessary, we then iteratively removed random effects from the maximal model until it successfully converged with a full rank (i.e., non-singular) random effects variance-covariance matrix. 
...
However, in order to ensure consistency in our approach across the 15 separate GLMMs we fit in this set of analyses, we established a general approach to selecting a boundary-estimated parameter to eliminate for each stepwise reduction of the model's complexity, broadly adapted from~\mbox{%DIFAUXCMD
\citet{BateEtal15b}}\hskip0pt%DIFAUXCMD
.
...
Third, to choose which of two zero-estimated variance components of the }\textit{\DIFadd{same}} \DIFadd{order to remove from the model, we took one of two approaches.
