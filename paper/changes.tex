...
%DIF DEL old2.tex   Tue Oct 24 20:50:12 2023
%DIF ADD main.tex   Tue Oct 24 22:07:03 2023
...
(Fig.~\ref{fig:predictions}) suggests that spatial relationships \DIFdelbegin \DIFdel{in the text
embedding }\DIFdelend \DIFaddbegin \DIFadd{within this
}\DIFaddend space can help explain what participants know. But how far does this
explanatory power extend? For example, suppose we know that a participant
\DIFdelbegin \DIFdel{answers a question (}\DIFdelend \DIFaddbegin \DIFadd{correctly answered a question }\DIFaddend at embedding coordinate $x$\DIFdelbegin \DIFdel{) correctly}\DIFdelend . As we move \DIFaddbegin \DIFadd{farther
}\DIFaddend away from $x$ in the embedding space, how does \DIFdelbegin \DIFdel{their knowledge }\DIFdelend \DIFaddbegin \DIFadd{the likelihood that the
participant knows about the content at a given location }\DIFaddend ``fall off'' with
distance? \DIFdelbegin \DIFdel{Or}\DIFdelend \DIFaddbegin \DIFadd{Conversely}\DIFaddend , suppose the participant instead answered that same
question \textit{in}correctly. Again, as we move \DIFaddbegin \DIFadd{farther }\DIFaddend away from $x$ in the
embedding space, how \DIFdelbegin \DIFdel{do the chances }\DIFdelend \DIFaddbegin \DIFadd{does the likelihood }\DIFaddend that the participant does \textit{not}
know about \DIFdelbegin \DIFdel{the
}\DIFdelend \DIFaddbegin \DIFadd{a coordinate's }\DIFaddend content change with distance? We reasoned that,
assuming our \DIFaddbegin \DIFadd{embedding }\DIFaddend space is capturing something about how \DIFdelbegin \DIFdel{participants }\DIFdelend \DIFaddbegin \DIFadd{individuals
}\DIFaddend actually organize their knowledge, \DIFdelbegin \DIFdel{conceptual
knowledge right around }\DIFdelend \DIFaddbegin \DIFadd{a participant's ability to answer questions
embedded very close to }\DIFaddend $x$ should \DIFaddbegin \DIFadd{tend to }\DIFaddend be similar to \DIFdelbegin \DIFdel{the participant's knowledge of
the content at }\DIFdelend \DIFaddbegin \DIFadd{their ability to answer
the question embedded }\textit{\DIFadd{at}} \DIFaddend $x$. \DIFdelbegin \DIFdel{And }\DIFdelend \DIFaddbegin \DIFadd{Whereas }\DIFaddend at another extreme, \DIFdelbegin \DIFdel{at some distance (after moving
sufficiently far away }\DIFdelend \DIFaddbegin \DIFadd{once we
reach some sufficiently large distance }\DIFaddend from $x$\DIFdelbegin \DIFdel{), our guesses about what participants know
(}\DIFdelend \DIFaddbegin \DIFadd{, our ability to infer whether
or not a participant will correctly answer a question }\DIFaddend based on their \DIFdelbegin \DIFdel{response to
}\DIFdelend \DIFaddbegin \DIFadd{ability to
answer }\DIFaddend the question at \DIFdelbegin \DIFdel{location }\DIFdelend $x$ \DIFdelbegin \DIFdel{) }\DIFdelend should be no better than guessing based on their
\DIFdelbegin \DIFdel{overall }\DIFdelend \DIFaddbegin \textit{\DIFadd{overall}} \DIFaddend proportion of correctly answered questions\DIFdelbegin \DIFdel{—i.e.,
if $y$ is very far away from $x$, all we can do with the }\DIFdelend \DIFaddbegin \DIFadd{. In other words,
beyond the maximum distance at which the }\DIFaddend participant's \DIFdelbegin \DIFdel{response to }\DIFdelend \DIFaddbegin \DIFadd{ability to answer the
question at }\DIFaddend $x$ is \DIFdelbegin \DIFdel{guess that ``their performance on quiz
questions about }\DIFdelend \DIFaddbegin \DIFadd{informative of their ability to answer a second question at
location }\DIFaddend $y$\DIFdelbegin \DIFdel{will be about equal to their average performance on quiz
questions about any material.
''
}\DIFdelend \DIFaddbegin \DIFadd{, then guessing the outcome at $y$ based on $x$ should be no more
...
contained in the two lectures. \DIFdelbegin \DIFdel{This change in the number of topics overcame an
...
substantially less clumping (Fig.~\ref{fig:knowledge-maps}). }\DIFdelend We note that we used these 2D maps solely for
...
Stars}).
\DIFdelbegin \DIFdel{While we are not aware of any specific confounds of viewing order, nor
...
(covering material that was not presented in either video). \DIFdelbegin \DIFdel{One of our group's
...
additional questions about general physics knowledge.'' }\DIFdelend To help broaden the
set of lecture-specific questions, \DIFdelbegin \DIFdel{the researchers were further instructed to work
}\DIFdelend \DIFaddbegin \DIFadd{our team worked }\DIFaddend through each lecture in
small segments \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend identify what each segment was ``about'' conceptually, and
then write a question about that concept. The general physics questions were
drawn \DIFdelbegin \DIFdel{from the researchers'coursework }\DIFdelend \DIFaddbegin \DIFadd{our team's prior coursework and areas of interest, }\DIFaddend along with internet
searches and brainstorming with the project team and other members of J.R.M.'s
lab. \DIFdelbegin \DIFdel{The final set of questions (and response options) was
reviewed and approved by J.R.M. before we collected or analyzed the text or
experimental data.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{Although we attempted to design the questions to test ``conceptual
knowledge,'' we }\DIFaddend note that estimating the specific ``amount'' of conceptual
...
behavior in the UMAP embedding procedure~\mbox{%DIFAUXCMD
\citep{McInEtal18a}}\hskip0pt%DIFAUXCMD
, whereby embedding
