...
%DIF ADD main.tex   Mon Feb 19 17:29:53 2024
...
student ``knows'' about the content of a given lecture entail \DIFaddbegin \DIFadd{administering some form of assessment (e.g., a quiz) and }\DIFaddend computing the
proportion of correctly answered questions. But if two students receive
identical scores on \DIFaddbegin \DIFadd{such }\DIFaddend an exam, might our modeling framework help us to gain more
...
\DIFdelbegin \DIFdel{,
}\DIFdelend $x$ \DIFdelbegin \DIFdel{, }\DIFdelend in text embedding space (e.g., the content reflected by any moment in a
...
\DIFdelbegin \DIFdel{average
}\DIFdelend proportion of quiz questions the participant answered correctly, where the
...
describing how much ``knowledge'' \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend participant has about the content
...
\begin{figure}[tp]
    ...
    participants. \DIFdelbeginFL \textbf{\DIFdelFL{B. Average estimated knowledge about }\textbf{\DIFdelFL{Four
    Fundamental Forces}}%DIFAUXCMD
\DIFdelFL{.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{B. Average estimated knowledge about }\textit{\DIFaddFL{Four
    Fundamental Forces}}\DIFaddFL{.}} \DIFaddendFL Each bar displays the across-timepoint average
...
carry some predictive information about whether \DIFdelbegin \DIFdel{the participant is }\DIFdelend \DIFaddbegin \DIFadd{they are }\DIFaddend likely to answer that
question correctly or incorrectly. We developed a statistical approach to test
this claim. For each \DIFdelbegin \DIFdel{question}\DIFdelend \DIFaddbegin \DIFadd{quiz question a participant answered}\DIFaddend , in turn, we used
Equation~\ref{eqn:prop} to \DIFdelbegin \DIFdel{predict each participant's }\DIFdelend \DIFaddbegin \DIFadd{estimate their }\DIFaddend knowledge at the given question's
embedding space coordinate \DIFdelbegin \DIFdel{, using all }\textit{\DIFdel{other}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{based on other }\DIFaddend questions that participant answered
on the same quiz. \DIFdelbegin \DIFdel{For each }\DIFdelend \DIFaddbegin \DIFadd{We repeated this for all participants, and for each of the
three quizzes. Then, separately for each }\DIFaddend quiz, we \DIFdelbegin \DIFdel{grouped these
predicted knowledge values into two distributions: one for the predicted knowledge at the
coordinates of
}\textit{\DIFdel{correctly}} %DIFAUXCMD
\DIFdel{answered questions, and another for the
predicted knowledge at the coordinates of }\textit{\DIFdel{incorrectly}} %DIFAUXCMD
\DIFdel{answered
questions (Fig.~\ref{fig:predictions}). We then used Mann-Whitney $\U$-tests to
compare the means of
these distributions of predicted knowledge }\DIFdelend \DIFaddbegin \DIFadd{fit a generalized linear
...
and questions (see }\nameref{subsec:glmm}\DIFadd{). To assess the predictive value of
...
bootstrap likelihood-ratio tests}\DIFaddend .

\begin{figure}[tp]
    \centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\textwidth]{figs/predict-knowledge-questions}
%DIFDELCMD < %%%
\DIFdelendFL 

    %DIF >  TODO: adjust width to max possible based on finalized caption
    \DIFaddbeginFL \includegraphics[width=0.75\textwidth]{figs/predict-knowledge-questions}
    \DIFaddendFL \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Predicting knowledge at the embedding coordinates of
    held-out questions.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Predicting success on held-out questions using estimated
    knowledge.}} \DIFaddFL{We used generalized linear mixed models (GLMMs) to model the
    likelihood of correctly answering a quiz question as a function of
    estimated knowledge for its embedding coordinate (see
    }\nameref{subsec:glmm}\DIFaddFL{). }\DIFaddendFL Separately for each quiz (column), we \DIFdelbeginFL \DIFdelFL{plot the
    distributions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{examined this
    relationship based on three different sets }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{predicted }\DIFdelendFL knowledge \DIFdelbeginFL \DIFdelFL{at the embedding coordinates of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{estimates:
    knowledge for }\DIFaddendFL each \DIFdelbeginFL \DIFdelFL{held-out correctly (blue) or incorrectly (red) answered }\DIFdelendFL question \DIFdelbeginFL \DIFdelFL{. The
    Mann-Whitney $\U$-tests reported in each panel are between }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{based on all other questions }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{distributions of predicted knowledge at the coordinates of correctly and
    incorrectly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same
    participant }\DIFaddendFL answered \DIFdelbeginFL \DIFdelFL{held-out questions. In }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{top row }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same quiz }\DIFaddendFL (``All questions''\DIFaddbeginFL \DIFaddFL{; top row}\DIFaddendFL ),
    \DIFdelbeginFL \DIFdelFL{we used all quiz questions (from each quiz, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{knowledge }\DIFaddendFL for each \DIFdelbeginFL \DIFdelFL{participant) except one to predict knowledge at the held-out }\DIFdelendFL question \DIFdelbeginFL \DIFdelFL{'s
    embedding coordinate. In the middle rows (``Across-lecture''), we used all
    questions }\DIFdelendFL about one lecture \DIFdelbeginFL \DIFdelFL{to predict knowledge at }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{based on all questions (from
    }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{embedding
    coordinate of a held-out question }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same participant and quiz) }\DIFaddendFL about the \textit{other} lecture
    \DIFdelbeginFL \DIFdelFL{. In the
    bottom row }\DIFdelendFL (``\DIFdelbeginFL \DIFdelFL{Within-lecture}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Across-lecture}\DIFaddendFL ''\DIFaddbeginFL \DIFaddFL{; middle rows}\DIFaddendFL ), \DIFdelbeginFL \DIFdelFL{we used all but one }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and knowledge for each }\DIFaddendFL question about
    one lecture \DIFdelbeginFL \DIFdelFL{to predict knowledge at }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{based on all other questions (from }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{embedding coordinate of a held-out
    question }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same participant and
    quiz) }\DIFaddendFL about the \textit{same} lecture \DIFaddbeginFL \DIFaddFL{(``Within-lecture''; bottom rows)}\DIFaddendFL .
    \DIFdelbeginFL \DIFdelFL{We repeated each of these
    analyses using all possible held-out questions for each quiz and
    participant. }\DIFdelendFL The \DIFdelbeginFL \DIFdelFL{arrows at the tops of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{background in }\DIFaddendFL each panel \DIFdelbeginFL \DIFdelFL{indicate whether }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{displays }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{average predicted knowledge was higher for held-out }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{kernel density estimates of the
    relative observed proportions of }\DIFaddendFL correctly \DIFdelbeginFL \DIFdelFL{answered
    }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{left}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{blue}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{or }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{versus }\DIFaddendFL incorrectly \DIFdelbeginFL \DIFdelFL{answered }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{right}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{red}\DIFaddendFL )
    \DIFaddbeginFL \DIFaddFL{answered }\DIFaddendFL questions\DIFaddbeginFL \DIFaddFL{, for each level of estimated knowledge along the
    $x$-axis}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{The black curves display the (population-level) GLMM-predicted
    probabilities of correctly answering a question as a function of estimated
    knowledge. Error ribbons denote bootstrap-estimated 95\% confidence
    intervals. }\textbf{\DIFaddFL{JRM NOTE: RE-ORDER ROWS TO MATCH THE TEXT, AND UPDATE
    CAPTION ACCORDINGLY}}\DIFaddendFL }

    \label{fig:predictions}
\end{figure}

We carried out \DIFdelbegin \DIFdel{these analyses in three different ways. First,
we used all (but one) of the questions from a given quiz (and participant) to predict knowledge
at the
embedding coordinate of a held-out question }\DIFdelend \DIFaddbegin \DIFadd{three different versions of the analyses described above,
...
at each question's embedding coordinate using }\textit{\DIFadd{all}} \DIFadd{other questions
answered by the same participant on the same quiz }\DIFaddend (``All questions''\DIFdelbegin \DIFdel{in
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend Fig.~\ref{fig:predictions}\DIFaddbegin \DIFadd{, top row}\DIFaddend ). This test was intended to \DIFdelbegin \DIFdel{serve as an overall
baseline for the
}\DIFdelend \DIFaddbegin \DIFadd{assess the
overall }\DIFaddend predictive power of our approach. Second, we \DIFdelbegin \DIFdel{used questions
...
\DIFdel{lecture , }\DIFdelend \DIFaddbegin \DIFadd{estimated knowledge for
each question about a given lecture using only the other questions (}\DIFaddend from the
same \DIFdelbegin \DIFdel{quiz and participant (``Across-lecture''in }\DIFdelend \DIFaddbegin \DIFadd{participant and quiz) about that }\textit{\DIFadd{same}} \DIFadd{lecture
(``Within-lecture''; }\DIFaddend Fig.~\ref{fig:predictions}\DIFaddbegin \DIFadd{, middle rows}\DIFaddend ). This test was
intended to \DIFdelbegin \DIFdel{test the }\DIFdelend \DIFaddbegin \DIFadd{assess the }\DIFaddend \textit{\DIFdelbegin \DIFdel{generalizability}\DIFdelend \DIFaddbegin \DIFadd{specificity}\DIFaddend } of our approach by asking whether
our \DIFdelbegin \DIFdel{knowledge predictions held across the content
areas of the two lectures}\DIFdelend \DIFaddbegin \DIFadd{predictions could distinguish between questions about different content
covered by the same lecture}\DIFaddend . Third, we \DIFdelbegin \DIFdel{used questions about one lecture to predict knowledge at the embedding
coordinate of a held-out question
about the }\textit{\DIFdel{same}} %DIFAUXCMD
\DIFdel{lecture , }\DIFdelend \DIFaddbegin \DIFadd{estimated knowledge for each question
about one lecture using only questions (}\DIFaddend from the same \DIFdelbegin \DIFdel{quiz and participant (``Within-lecture''in }\DIFdelend \DIFaddbegin \DIFadd{participant and quiz)
about the }\textit{\DIFadd{other}} \DIFadd{lecture (``Across-lecture'';
}\DIFaddend Fig.~\ref{fig:predictions}\DIFaddbegin \DIFadd{, bottom rows}\DIFaddend ). This test was intended to \DIFdelbegin \DIFdel{test the
}\DIFdelend \DIFaddbegin \DIFadd{assess the
}\DIFaddend \textit{\DIFdelbegin \DIFdel{specificity}\DIFdelend \DIFaddbegin \DIFadd{generalizability}\DIFaddend } of our approach by asking whether our \DIFdelbegin \DIFdel{knowledge predictions
could distinguish between questions about
different content covered by the same lecture.
We repeated each of these analysesusing all possible held-out questions for each quiz and participant.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{For the initial quizzes participantstook (prior to watching either lecture),
predicted knowledge tended to be low overall, and relatively
unstructured (Fig. ~\ref{fig:predictions}, left column). When we held out individual questions
and predicted their knowledge at the held-out questions
'
embedding coordinates, we found no reliable differences in the }\DIFdelend predictions
\DIFaddbegin \DIFadd{held across the content areas of the two lectures.
}

\DIFadd{In performing these analyses, our null hypothesis is that the knowledge
estimates we compute based on the quiz questions' embedding coordinates do
}\textit{\DIFadd{not}} \DIFadd{provide useful information about participants' abilities to answer
...
}\textit{\DIFadd{correctly}} \DIFadd{answered question, the proportion of remaining questions
answered correctly would be $\frac{n - 1}{q - 1}$. If we hold out a single
}\textit{\DIFadd{incorrectly}} \DIFadd{answered question, the proportion of remaining questions
answered correctly would be $\frac{n}{q - 1}$. In this way, the proportion of
correctly answered remaining questions is always }\textit{\DIFadd{lower}} \DIFaddend when the
held-out question \DIFdelbegin \DIFdel{had been correctly versus incorrectly answered . This
``null'' effect persisted when we used }\textit{\DIFdel{all}} %DIFAUXCMD
\DIFdel{of the Quiz 1 questions
from a given participant to predict a }\DIFdelend \DIFaddbegin \DIFadd{was answered correctly than when it was answered incorrectly.
...
embedding-space distance from the }\DIFaddend held-out question\DIFdelbegin \DIFdel{(``All questions''; $\U
= 50587,~p = 0.723$),
when we
used questions from one lecture to predict
knowledge
at the embedding coordinate of a held-out question about the }\DIFdelend \DIFaddbegin \DIFadd{; see Eqn.~\ref{eqn:prop}),
if these weights were uninformative (e.g., randomly distributed), then we
...
among the quiz questions' embeddings }\textit{\DIFadd{are}} \DIFadd{predictive of participants'
knowledge about the questions' content, then we would expect }\DIFaddend \textit{\DIFdelbegin \DIFdel{other}\DIFdelend \DIFaddbegin \DIFadd{higher}\DIFaddend }
\DIFdelbegin \DIFdel{lecture (``Across-lecture''; predicting }\DIFdelend \DIFaddbegin \DIFadd{estimated }\DIFaddend knowledge for held-out \DIFdelbegin \textit{\DIFdel{Four Fundamental Forces Questions}} %DIFAUXCMD
\DIFdel{using }\textit{\DIFdel{Birth of Stars}}
%DIFAUXCMD
\DIFdel{questions: $\U = 8244,~p = 0.184$; predicting knowledge for held-out
}\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{questions: $\U = 8202.5,~p = 0.161$), and when we used
questions from one lecture to predict }\DIFdelend \DIFaddbegin \DIFadd{correctly (versus incorrectly) answered
questions.
}
...
with our null hypothesis: higher estimated }\DIFaddend knowledge at the embedding
coordinate of a held-out question \DIFdelbegin \DIFdel{about the }\textit{\DIFdel{same}} %DIFAUXCMD
\DIFdel{lecture (``Within-lecture'';
}\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{was associated with a lower likelihood of
...
general, or about Quiz~1 performance in particular. When we repeated this
analysis for Quizzes~2 and~3, the direction of this relationship reversed:
higher estimated knowledge for a given question predicted a greater likelihood
of answering it correctly (Quiz~2}\DIFaddend : \DIFdelbegin \DIFdel{$\U = 7681.5,~p = 0.746$; }\textit{\DIFdel{Birth of
Stars}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{$OR = 2.905,\ \lambda_{LR} = 17.333,\ 95\%\
\textnormal{CI} = [14.966,\ 29.309],\ p = 0.002$; Quiz~3}\DIFaddend : \DIFdelbegin \DIFdel{$\U = 8125,~p = 0.204$). We believe that this
reflects a floor effect:
when knowledge is low everywhere, there is little signal to differentiate
between what is known versus unknown. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{After watching }\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdel{, predicted knowledge for held-out questions that were answered correctly (from the second quiz; Fig.~\ref{fig:predictions}, middle column)exhibited a significant positive
shift relative to held-out questions that were answered incorrectly.
This held
when we included all questionsin the analysis ($\U = 58332,~p < 0.001$), when
we predicted knowledge across-lectures (}\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdel{: $\U =
6749.5,~p = 0.014$; }\textit{\DIFdel{Birth of Stars}}%DIFAUXCMD
\DIFdel{: $\U = 8480,~p = 0.016$}\DIFdelend \DIFaddbegin \DIFadd{$OR = 3.238,\
\lambda_{LR} = 6.882,\ 95\%\ \textnormal{CI} = [6.228,\ 8.184],\ p = 0.017$).
Taken together, these results suggest that our knowledge estimates }\textit{\DIFadd{do}}
\DIFadd{reliably predict participants' likelihood of success on individual quiz
questions, but only when there is some ``contrast'' in performance between the
content of different questions. On Quiz~1, before participants have learned the
content of either lecture, their overall performance is poor (average
proportion correct: $0.46$), and the specific questions they answer correctly
not not seem to be structured in a meaningful way with respect to the embedding
space (e.g., Fig.~\ref{fig:knowledge-maps}A, left panel). On Quizzes~2 and~3,
after participants have begun their training, their overall performance
improves (average proportion correct for Quiz~2: 0.60; average proportion
correct for Quiz~3: 0.79}\DIFaddend ), and \DIFdelbegin \DIFdel{when
we predicted knowledge at the }\DIFdelend \DIFaddbegin \DIFadd{the specific content areas they improve on are
captured by the lecture and question embeddings (and, therefore, our knowledge
estimates).
}

\DIFadd{We observed a similar pattern of results when we fit GLMMs to estimates of
participants' knowledge for each question about one lecture derived from other
questions about the }\textit{\DIFadd{same}} \DIFadd{lecture. Specifically, for questions that
participants answered on Quiz~1, prior to watching either lecture, knowledge
for the }\DIFaddend embedding coordinates of \DIFdelbegin \DIFdel{held-out }\DIFdelend \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{questions }\DIFdelend \DIFaddbegin \DIFadd{-related
questions estimated }\DIFaddend using other \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{questions from the same quiz and participant ($\U = 7224,~p < 0.001$). This
difference did }\textit{\DIFdel{not}} %DIFAUXCMD
\DIFdel{hold for within-lecture knowledge predictions at
knowledge at embedding space coordinates of }\DIFdelend \DIFaddbegin \DIFadd{-related
questions did not reliably predict whether those questions were answered
correctly ($OR = 1.891,\ \lambda_{LR} = 2.293,\ 95\%\ \textnormal{CI} =
[2.091,\ 2.622],\ p = 0.139$). The same was true of knowledge estimates for
}\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{questions
($\U = 7419,~p = 0.739$). Again, we suggest that this might reflect a floor
effect whereby, at that point in the participants' training, their knowledge about the content of the }\DIFdelend \DIFaddbegin \DIFadd{-related questions based on other }\DIFaddend \textit{Birth of
Stars}\DIFdelbegin \DIFdel{material is relatively low
everywhere in that region of text embedding space.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Finally, after watching }\DIFdelend \DIFaddbegin \DIFadd{-related questions ($OR = 0.722,\ \lambda_{LR} = 5.115,\ 95\%\
\textnormal{CI} = [0.094,\ 0.146],\ p = 0.738$). When we recomputed these
within-lecture knowledge estimates using questions from Quiz~2---which
participants took immediately after viewing }\textit{\DIFadd{Four Fundamental Forces}}
\DIFadd{but prior to viewing }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{, predicted knowledge for
...
predictions (}\DIFdelend \DIFaddbegin \DIFadd{---we found that they now reliably
predicted success on }\DIFaddend \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{: $\U = 6684.5,~p = 0.032$;
}\DIFdelend \DIFaddbegin \DIFadd{-related questions ($OR =
9.023,\ \lambda_{LR} = 18.707,\ 95\%\ \textnormal{CI} = [10.877,\ 22.222],\ p =
0.001$) but not on }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{: $\U = 6414.5,~p = 0.002$), and and when we carried out
}\DIFdelend \DIFaddbegin \DIFadd{-related questions ($OR = 0.306,\
\lambda_{LR} = 5.115,\ 95\%\ \textnormal{CI} = [4.624,\ 5.655],\ p = 0.055$).
Using participants' responses from Quiz~3 (taken immediately after viewing
}\textit{\DIFadd{Birth of Stars}}\DIFadd{), we found that }\DIFaddend within-lecture knowledge \DIFdelbegin \DIFdel{predictions for
held-out }\DIFdelend \DIFaddbegin \DIFadd{estimates for
}\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{questions using other }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
\DIFdel{questions from the same quiz and
participant ($\U = 6126,~p = 0.006$). However, we found the }\textit{\DIFdel{opposite}}
%DIFAUXCMD
\DIFdel{effect when we carried out }\DIFdelend \DIFaddbegin \DIFadd{-related questions could now reliably predict success on
those questions ($OR = 5.467,\ \lambda_{LR} = 10.670,\ 95\%\ \textnormal{CI} =
[7.998, 12.532],\ p = 0.006$). In contrast, }\DIFaddend within-lecture knowledge \DIFdelbegin \DIFdel{predictions for held-out
}\DIFdelend \DIFaddbegin \DIFadd{estimates
for }\DIFaddend \textit{Four Fundamental Forces} questions \DIFdelbegin \DIFdel{using other }\DIFdelend \DIFaddbegin \DIFadd{answered on Quiz~3 were no
longer directly related to the likelihood of successfully answering them and
instead exhibited the inverse relationship we would expect to arise from
unstructured knowledge (with respect to the embedding space; $OR = 0.013,\
\lambda_{LR} = 14.648,\ 95\%\ \textnormal{CI} = [10.695, 23.096],\ p = 0.001$).
In principle, this ``prediction failure'' might arise from several possible
scenarios. One possibility is that the participants are answering nearly all of
the questions about }\DIFaddend \textit{Four Fundamental Forces} \DIFdelbegin \DIFdel{questions from the same quiz and
participant ($\U = 6734,~p = 0.027$).
Specifically, on Quiz }\DIFdelend \DIFaddbegin \DIFadd{correctly on Quiz~}\DIFaddend 3, \DIFdelbegin \DIFdel{our knowledgepredictions for held-out correctly
answered }\DIFdelend \DIFaddbegin \DIFadd{and
(as with the Quiz~1 questions) there might be insufficient contrast in
performance across different questions to reliably estimate participants'
knowledge. However, when we examined participants' performance on Quiz~3
}\DIFaddend questions about \textit{Four Fundamental Forces}\DIFdelbegin \DIFdel{were reliably
}\textit{\DIFdel{lower}} %DIFAUXCMD
\DIFdel{than those for their incorrectly answered
counterparts. Speculatively, we suggest that this may reflect participants
}\DIFdelend \DIFaddbegin \DIFadd{, we found that they answered
these questions correctly only 76\% of the time, on average (i.e., unlikely to
reflect a ``ceiling'' effect). Alternatively, we speculated that participants
might be }\DIFaddend forgetting some of the \DIFaddbegin \DIFadd{material from the }\DIFaddend \textit{Four Fundamental
Forces} \DIFdelbegin \DIFdel{content. If this forgetting happens }\DIFdelend \DIFaddbegin \DIFadd{lecture, and that this forgetting might be happening }\DIFaddend in a relatively
``random'' way \DIFdelbegin \DIFdel{(}\DIFdelend with respect to spatial distance within the \DIFdelbegin \DIFdel{text
embedding space), then it could explain why some held-out questions about
}\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{were answered incorrectly, even if questions
at nearby coordinates (i.e., about similar content) were answered correctly}\DIFdelend \DIFaddbegin \DIFadd{embedding space}\DIFaddend .
This might lead our approach to over-estimate knowledge for held-out questions
about ``forgotten'' knowledge that participants answered incorrectly. \DIFaddbegin \DIFadd{Indeed,
participants' performance on }\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{-related questions
was slightly (thought not dramatically) lower on Quiz~3 than on Quiz~2 (mean
proportion correct on Quiz~2: $0.77$). }\DIFaddend Taken together, \DIFdelbegin \DIFdel{the results in Figure~\ref{fig:predictions} indicate }\DIFdelend \DIFaddbegin \DIFadd{these results suggest
}\DIFaddend that our approach can \DIFdelbegin \DIFdel{reliably predict acquired knowledge (especially about recently
learned content) , and that }\DIFdelend \DIFaddbegin \DIFadd{distinguish between questions about different content
covered by a single lecture when participants have sufficiently structured
knowledge about its contents, though this specificity may decrease further in
time from when the lecture in question was viewed.
}

\DIFadd{Finally, when we fit GLMMs to estimates of participants' knowledge for
questions about one lecture using questions they answered (on the same quiz)
about }\DIFaddend the \DIFdelbegin \DIFdel{knowledge predictions are generalizable across
the content areas spanned by the two lectures, while also specific enough to }\DIFdelend \DIFaddbegin \textit{\DIFadd{other}} \DIFadd{lecture, we also observed a (largely) similar pattern.
On Quiz~1, we found that participants' abilities to correctly answer questions
about }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{could be predicted from their responses
to questions about }\textit{\DIFadd{Birth of Stars}} \DIFadd{($OR = 1.896,\ \lambda_{LR} =
7.205,\ 95\%\ \textnormal{CI} = [6.224, 7.524],\ p = 0.039$) and similarly,
that their ability to correctly answer }\textit{\DIFadd{Birth of Stars}}\DIFadd{-related
questions could be predicted from their responses to }\textit{\DIFadd{Four Fundamental
Forces}}\DIFadd{-related questions ($OR = 1.522,\ \lambda_{LR} = 6.448,\ 95\%\
\textnormal{CI} = [5.656, 6.843],\ p = 0.043$). This reflects coarse-scale
structure in participants' knowledge prior to any training in our experiment.
When we repeated this analysis using questions from Quiz~2, we found
participants' responses to }\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{-related questions
did }\textit{\DIFadd{not}} \DIFadd{reliably predict their success on }\textit{\DIFadd{Birth of
Stars}}\DIFadd{-related questions ($OR = 1.865,\ \lambda_{LR} = 3.205,\ 95\%\
\textnormal{CI} = [3.027, 3.600],\ p = 0.125$), nor did their responses to
}\textit{\DIFadd{Birth of Stars}}\DIFadd{-related questions reliably predict their success on
}\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{-related questions ($OR = 3.490,\ \lambda_{LR}
= 3.266,\ 95\%\ \textnormal{CI} = [3.033, 3.866],\ p = 0.094$). These
``prediction failures'' appear to come from the fact that any signal derived
from participants' knowledge about the content of the }\textit{\DIFadd{Birth of Stars}}
...
for }\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{-related questions on Quiz~2: $0.77$; mean
proportion correct for }\textit{\DIFadd{Birth of Stars}}\DIFadd{-related questions on Quiz~2:
$0.36$). When we again carried out these across-lecture knowledge predictions
using questions from Quiz~3 (when participants had now viewed }\textit{\DIFadd{both}}
...
abilities to answer individual quiz questions, }\DIFaddend distinguish between questions
about \DIFdelbegin \DIFdel{more subtly different contentwithin the
same lecture}\DIFdelend \DIFaddbegin \DIFadd{similar content, and generalize across content areas, provided that
...
are made. Our results also indicate some potential limitations of our approach:
when the contrast in participants' knowledge within the embedding space is low
overall, our approach (often) cannot reliably predict their abilities to answer
individual quiz questions}\DIFaddend .
...
coordinates. \DIFdelbegin \DIFdel{The ``spatial smoothness'' of }\DIFdelend \DIFaddbegin \DIFadd{How }\DIFaddend participants' knowledge \DIFdelbegin \DIFdel{(as
estimated using quiz performance) is
being }\DIFdelend \DIFaddbegin \DIFadd{falls off with spatial distance is
}\DIFaddend captured by the knowledge maps we \DIFdelbegin \DIFdel{are inferring }\DIFdelend \DIFaddbegin \DIFadd{infer }\DIFaddend from their quiz responses
...
\DIFaddbegin \subsubsection*{\DIFadd{GLMM METHODS SECTION PLACEHOLDER}}\label{subsec:glmm}
