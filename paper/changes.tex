...
%DIF ADD main.tex   Tue Oct 24 01:19:23 2023
...
%DIF 27a28-29
\newcommand{\ldaVsBERT}{8} %DIF > 
\newcommand{\jaccard}{9}   % word match figure %DIF > 
%DIF -------
...
experience~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{BlayEtal06,CaraMaho03, ConsEtal16, DeacEtal04, SimoEtal04}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{BlayEtal06,CaraMaho03, ConsEtal16, DeacEtal04, SimoEtal04,
HuebWill18}}\hskip0pt%DIFAUXCMD
}\DIFaddend ? Or weaving a lecture's atomic elements (e.g., its component words)
into a structured network that describes how those individual elements are
related~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{LeeChen22}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{LeeChen22, vanPEtal21}}\hskip0pt%DIFAUXCMD
}\DIFaddend ? Conceptual understanding could also
...
gestalt whole~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Kint70,
Macl05, ScotEtal07}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Kint70, Macl05, ScotEtal07, TulcEtal23}}\hskip0pt%DIFAUXCMD
}\DIFaddend .

The difference between ``understanding'' and ``memorizing,'' as framed by
researchers in education, cognitive psychology, and cognitive neuroscience
(e.g.,~\citealp{Kato40, Gall00, ScotEtal07, HallGree08, Macl05}), has profound
analogs in the fields of natural language processing and natural language
understanding. For example, considering the raw contents of a document (e.g.,
its constituent symbols, letters, and words) might provide some clues as to
what the document is about, just as memorizing a passage might provide some
ability to answer simple questions about it. However, text embedding models
(e.g.,~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citealp{LandDuma97, DeerEtal90, BleiEtal03,
BleiLaff06, MikoEtal13a, CerEtal18, BrowEtal20}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citealp{LandDuma97, DeerEtal90, BleiEtal03, BleiLaff06, MikoEtal13a,
CerEtal18, BrowEtal20, ViswEtal17}}\hskip0pt%DIFAUXCMD
}\DIFaddend ) also attempt to capture the deeper meaning
\textit{underlying} those atomic elements. These models consider not only the
co-occurrences of those elements within and across documents, but \DIFaddbegin \DIFadd{(in many
cases) }\DIFaddend also patterns in how those elements appear across different scales
(e.g., sentences, paragraphs, chapters, etc.), the temporal and grammatical
properties of the elements, and other high-level characteristics of how they
are used~\citep{Mann20, Mann21a}. According to these models, the deep
conceptual meaning of a document may be captured by a feature vector in a
high-dimensional representation space, wherein nearby vectors reflect
conceptually related documents. A model that succeeds at capturing an analogue
of ``understanding'' is able to assign nearby feature vectors to two
conceptually related documents, \textit{even when the specific words contained
in those documents have very little overlap}. \DIFaddbegin \DIFadd{In this way, ``concepts'' are
defined implicitly by the model's geometry~\mbox{%DIFAUXCMD
\citep[e.g., how the embedding coordinate
of a given word or document relates to the coordinates of other text
embeddings;][]{PianHill22}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

Given these insights, what form might a representation of the sum total of a
person's knowledge take \DIFaddbegin \DIFadd{(speculatively)}\DIFaddend ? First, we might require a means of
systematically describing or representing the nearly infinite set of possible
...
\DIFaddbegin \DIFadd{It is important to clarify that although we use topic model-derived embeddings
to }\textit{\DIFadd{characterize}} \DIFadd{the conceptual content of the lectures and questions,
we do not claim that the topic model itself }\textit{\DIFadd{understands}} \DIFadd{the conceptual
content of the lectures or questions. Rather, we view the topic model as a tool
for capturing the }\textit{\DIFadd{structure}} \DIFadd{of the conceptual content of the lectures
and questions in a way that enables us to capture, quantify, and track and
predict participants' knowledge.
}
...
\begin{figure}[tp]
    ...
    \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Estimating moment-by-moment knowledge acquisition.}}
    %DIFAUXCMD
\textbf{\DIFdelFL{A. Moment-by-moment knowledge about the }\textit{\DIFdelFL{Four Fundamental
    Forces}}%DIFAUXCMD
}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Estimating knowledge about the content presented at each
    moment of each lecture.}} \textbf{\DIFaddFL{A. Knowledge about the time-varying
    content of }\textit{\DIFaddFL{Four Fundamental Forces}}}\DIFaddendFL . Each trace displays the
    ...
    participants. \DIFdelbeginFL \textbf{\DIFdelFL{B.
    Average estimated knowledge about the }\textbf{\DIFdelFL{Four Fundamental Forces}}%DIFAUXCMD
\DIFdelFL{.}}
    %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{B. Average estimated knowledge about }\textbf{\DIFaddFL{Four
    ...
    \DIFdelbeginFL \textbf{\DIFdelFL{C. Moment-by-moment
    knowledge about the }\textit{\DIFdelFL{Birth of Stars}}%DIFAUXCMD
}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{C. Knowledge about the time-varying content of }\textit{\DIFaddFL{Birth of
    Stars}}}\DIFaddendFL . The panel is in the same format as Panel A, but here the knowledge
    estimates are for the moment-by-moment content of the \textit{Birth of
    Stars} lecture. \DIFdelbeginFL \textbf{\DIFdelFL{D.
    Average estimated knowledge about the }\textit{\DIFdelFL{Birth of Stars}}%DIFAUXCMD
\DIFdelFL{.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{D. Average estimated knowledge about }\textit{\DIFaddFL{Birth
    of Stars}}\DIFaddFL{.}} \DIFaddendFL The panel is in the same format as Panel B, but here the
...
\begin{figure}[tp]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figs/held-out-question-analysis}
%DIFDELCMD < %%%
...
\DIFaddendFL 
    ...
    \DIFdelbeginFL \DIFdelFL{$t$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Mann-Whitney $U$}\DIFaddendFL -tests reported in each panel are between the distributions
    of estimated knowledge at the coordinates of correctly versus incorrectly
    answered held-out questions. \DIFaddbeginFL \DIFaddFL{In the top row (``All questions''), we used
    all quiz questions (from each quiz, for each participant) except one to
    estimate knowledge at the held-out question's embedding coordinate. In the
    middle rows (``Across-lecture''), we used all questions about one lecture
    to estimate knowledge at the embedding coordinate of a held-out question
    about the }\textit{\DIFaddFL{other}} \DIFaddFL{lecture. In the bottom row (``Within-lecture''),
    we used all but one question about one lecture to estimate knowledge at the
    embedding coordinate of a held-out question about the }\textit{\DIFaddFL{same}}
    \DIFaddFL{lecture. We repeated each of these analyses using all possible held-out
    questions for each quiz and participant.}\DIFaddendFL }
...
unstructured (Fig.~\ref{fig:predictions}, left \DIFdelbegin \DIFdel{panel}\DIFdelend \DIFaddbegin \DIFadd{column}\DIFaddend ). When we held out
...
the held-out question had been correctly versus incorrectly answered\DIFdelbegin \DIFdel{($t(633) =
0.577,~p = 0.564$). This }\DIFdelend \DIFaddbegin \DIFadd{. This
``null'' effect persisted when we used }\textit{\DIFadd{all}} \DIFadd{of the Quiz 1 questions
...
}\textit{\DIFadd{other}} \DIFadd{lecture (``Across-lecture''; predicting knowledge for held-out
}\textit{\DIFadd{Four Fundamental Forces Questions}} \DIFadd{using }\textit{\DIFadd{Birth of Stars}}
\DIFadd{questions: $U = 8244, p = 0.184$; predicting knowledge for held-out
}\textit{\DIFadd{Birth of Stars}} \DIFadd{questions: $U = 8202.5, p = 0.161$), and when we used
questions from one lecture to predict knowledge at the embedding coordinate of
a held-out question about the }\textit{\DIFadd{same}} \DIFadd{lecture (``Within-lecture'';
}\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{: $U = 7681.5, p = 0.746$; }\textit{\DIFadd{Birth of
Stars}}\DIFadd{: $U = 8125, p = 0.204$). We believe that this }\DIFaddend reflects a floor effect:
when knowledge is low everywhere, there is little signal to differentiate
between what is known versus unknown. 
\DIFdelbegin \DIFdel{After watching the first lecture}\DIFdelend \DIFaddbegin 

\DIFadd{After watching }\textit{\DIFadd{Four Fundamental Forces}}\DIFaddend , estimated knowledge for
held-out correctly answered questions (from the second quiz;
Fig.~\ref{fig:predictions}, middle \DIFdelbegin \DIFdel{panel}\DIFdelend \DIFaddbegin \DIFadd{column}\DIFaddend ) exhibited a positive shift relative
to held-out incorrectly answered questions\DIFaddbegin \DIFadd{. This held when we included all
questions in the analysis ($U = 58332, p < 0.001$), when we predicted knowledge
across-lectures }\DIFaddend (\DIFdelbegin \DIFdel{$t(633) = 3.961,~p < 0.001$). This second quiz provides the maximally sensitive test for our knowledge
predictions, since (if knowledge is estimated accurately)participants' Quiz~2
responses should demonstrate specific knowledge about Lecture~1 content }\DIFdelend \DIFaddbegin \textit{\DIFadd{Four Fundamental Forces}}\DIFadd{: $U = 6749.5, p = 0.014$;
}\textit{\DIFadd{Birth of Stars}}\DIFadd{: $U = 8480, p = 0.016$), and when we predicted
knowledge at the embedding coordinates of held-out }\textit{\DIFadd{Four Fundamental
Forces}} \DIFadd{questions using other }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions from
the same quiz and participant ($U = 7224, p < 0.001$). This difference did
}\textit{\DIFadd{not}} \DIFadd{hold for within-lecture predictions of }\textit{\DIFadd{Birth of Stars}}
\DIFadd{knowledge ($U = 7419, p = 0.739$). Again, we suggest that this might reflect a
floor effect whereby knowledge about the content of the }\textit{\DIFadd{Birth of Stars}}
\DIFadd{material is relatively low everywhere in that region of text embedding space.
}

\DIFadd{Finally, after watching }\textit{\DIFadd{Birth of Stars}}\DIFaddend , \DIFdelbegin \DIFdel{but
knowledge about Lecture~2 and general physics concepts should be roughly
unchanged from before they watched Lecture~1. After watching the second lecture,
}\DIFdelend estimated knowledge \DIFaddbegin \DIFadd{for
held-out correctly answered questions }\DIFaddend (from the third quiz;
Fig.~\ref{fig:predictions}, right \DIFdelbegin \DIFdel{panel) for }\textit{\DIFdel{all}} %DIFAUXCMD
\DIFdel{questions exhibited a positive shift. However, the
estimated knowledge for }\DIFdelend \DIFaddbegin \DIFadd{column) was higher for }\DIFaddend held-out \DIFdelbegin \DIFdel{correctly answered questions remained
greater than that }\DIFdelend \DIFaddbegin \DIFadd{correctly
answered questions than for held-out incorrectly answered questions. This held
when we included all questions in the analysis ($U = 38279, p = 0.022$), when
we carried out across-lecture predictions (}\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{: $U
= 6684.5, p = 0.032$; }\textit{\DIFadd{Birth of Stars}}\DIFadd{: $U = 6414.5, p = 0.002$), and
and when we carried out within-lecture predictions of held-out }\textit{\DIFadd{Birth of
Stars}} \DIFadd{questions using other }\textit{\DIFadd{Birth of Stars}} \DIFadd{questions from the same
quiz and participant ($U = 6126, p = 0.006$). However, we found the
}\textit{\DIFadd{opposite}} \DIFadd{effect when we carried out within-lecture predictions of
held-out }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions using other }\textit{\DIFadd{Four
Fundamental Forces}} \DIFadd{questions from the same quiz and participant ($U = 6734, p
= 0.027$). Specifically, held-out correctly answered Quiz 3 questions about
}\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{had reliably }\textit{\DIFadd{lower}} \DIFadd{estimated
knowledge than held-out incorrectly answered questions. Speculatively, we
suggest that this may reflect participants forgetting some of the }\textit{\DIFadd{Four
Fundamental Forces}} \DIFadd{content. If this forgetting happens in a relatively
``random'' way (with respect to spatial distance within the text embedding
space), then it could explain why some held-out questions about }\textit{\DIFadd{Four
Fundamental Forces}} \DIFadd{were answered incorrectly, even if questions at nearby
coordinates (i.e., about similar content) were answered correctly. This might
lead our approach to over-estimate knowledge }\DIFaddend for held-out \DIFdelbegin \DIFdel{incorrectly }\DIFdelend \DIFaddbegin \DIFadd{questions about
``forgotten'' knowledge that particpants answered incorrectly.
}

\DIFadd{That the knowledge estimates derived from the text embedding space reliably
distinguish between held-out correctly versus incorrectly }\DIFaddend answered questions
(\DIFdelbegin \DIFdel{$t(628) = 2.045,~p
= 0.041$) }\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{fig:predictions}) suggests that the text embedding space bears at
least some relationship to participants' knowledge. But what does that
relationship look like as we move through the embedding space? For example,
suppose we know that a participant answers a question (at embedding coordinate
$X$) correctly. As we move away from $X$ in the embedding space, how does quiz
performance ``fall off'' with distance? Or, suppose the participant instead
answered that same question }\textit{\DIFadd{in}}\DIFadd{correctly. Again, as we move away from
$X$ in the embedding space, how does our confidence that the participant does
}\textit{\DIFadd{not}} \DIFadd{know about the content change with distance? We reasoned that,
assuming our space is capturing something about how participants actually
organize their knowledge, conceptual knowledge right around $X$ should be similar
to the participant's knowledge of the content at $X$. And at another extreme, at
some distance (after moving sufficiently far away from $X$), our guesses about
what participants know (based on their response to the question at location $X$)
should be no better than guessing based on their overall proportion of
correctly answered questions—i.e., if $Y$ is very far away from $X$, all we can do
with the participant's response to $X$ is guess that ``their performance on quiz
questions about $Y$ is about equal to their average performance on quiz questions
about any material.''
}
...
\begin{figure}[t]
    ...
    \caption{\textbf{\DIFaddFL{Quiz performance falls off gradually in text embedding
    space.}} \textbf{\DIFaddFL{A. Performance versus distance.}} \DIFaddFL{For each participant, for
    ...
    }\textit{\DIFaddFL{all}} \DIFaddFL{questions included in the analysis at the given distance.
    }\textbf{\DIFaddFL{B. Maximum distance for which performance is reliably different
    from the average.}} \DIFaddFL{We used a bootstrap procedure
    (see~}\nameref{subsec:smoothness}\DIFaddFL{) to estimate the point at which the blue
    ...
    from each quiz. }\textbf{\DIFaddFL{All panels.}} \DIFaddFL{Error ribbons denote
    bootstrap-estimated 95\% confidence intervals.}}

    \label{fig:smoothness}
\end{figure}

\DIFadd{With these ideas in mind, we asked: conditioned on answering a question
...
simple proportion correct score (see~}\nameref{subsec:smoothness}\DIFadd{). This
...
quizzes or regions of the embedding space}\DIFaddend .
\DIFdelbegin \DIFdel{This third contrast reflects a ceiling effect: when knowledge is relatively high everywhere, the signal differentiating what is known versus
unknown is relatively weak.Taken together, this set of analyses demonstrates
that our knowledge prediction framework is most informative when participantsexhibit variability in their knowledge of the content captured by the text
embedding model.
}\DIFdelend 
...
contained in the two lectures. \DIFaddbegin \DIFadd{This change in the number of topics overcame an
undesireable behavior in the UMAP embedding procedure~\mbox{%DIFAUXCMD
\citep{McInEtal18}}\hskip0pt%DIFAUXCMD
,
...
original 15-dimensional space, using the 15-topic model. }\DIFaddend Aside from increasing
...
\DIFaddbegin \DIFadd{We view our work as making several contributions to the study of how people
...
use these maps to predict responses to held-out questions
(Fig.~\ref{fig:predictions}) also has important psychological implications. One
such psychological implication is that concepts that are assigned to nearby
coordinates by the text embedding model also appear to be ``known to a similar
extent'' (as reflected by participants' responses to held-out questions). This
suggests that participants also }\textit{\DIFadd{conceptualize}} \DIFadd{similarly the content
reflected by nearby embedding coordinates. A second psychological implication
is that something about how participants' knowledge ``spreads'' across concepts
is being captured by the knowledge maps we are inferring from their quiz
responses (e.g., Figs.~\ref{fig:smoothness},~\ref{fig:knowledge-maps}). In
other words, our study shows that knowledge about a given concept implies
knowledge about related concepts.
}

\DIFadd{In our study, we characterize the ``coordinates'' of participants' knowledge
using a relatively simple ``bag of words'' text embedding model~\mbox{%DIFAUXCMD
\citep[LDA;
]{BleiEtal03}}\hskip0pt%DIFAUXCMD
. More sophisticated text embedding models, such as
transformer-based models~\mbox{%DIFAUXCMD
\citep{ViswEtal17, DevlEtal18, ChatGPT, TouvEtal23}
}\hskip0pt%DIFAUXCMD
can learn complex grammatical and semantic relationships between words,
higher-order syntactic structures, sylistic features, and more. We considered
using transformer-based models in our study, but we were surprised to find that
the text embeddings derived from these models were surprisingly uninformative
with respect to differentiating or otherwise characterizing the conceptual
content of the lectures and questions we used. We suspect that this reflects a
broader challenge in constructing models that are high-resolution within a
given domain (e.g., the domain of physics lectures and questions) }\textit{\DIFadd{and}}
\DIFadd{sufficiently broad so as to enable them to cover a wide range of domains. For
example, we found that the embeddings derived even from much larger and more
modern models like BERT~\mbox{%DIFAUXCMD
\citep{DevlEtal18}}\hskip0pt%DIFAUXCMD
, GPT~\mbox{%DIFAUXCMD
\citep{ViswEtal17}}\hskip0pt%DIFAUXCMD
,
LLaMa~\mbox{%DIFAUXCMD
\citep{TouvEtal23}}\hskip0pt%DIFAUXCMD
, and others that are trained on enormous text corpora,
end up yielding poor resolution within the content space spanned by indiviudal
course videos (Supp. Fig.~\ldaVsBERT). Whereas the LDA embeddings of the
lectures and questions are ``near'' each other (i.e., the convex hull enclosing
the two lectures' trajectories is highly overlapping with the convex hull
enclosing the questions' embeddings), the BERT embeddings of the lectures and
questions are instead largely distinct (top row of Supp. Fig.~\ldaVsBERT). The
LDA embeddings of the questions for each lecture and the corresponding
lecture's trajectory are also similar. For example, as shown in
Fig.~\ref{fig:sliding-windows}C, the LDA embeddings for }\textit{\DIFadd{Four
Fundamental Forces}} \DIFadd{questions (blue dots) appear closer to the }\textit{\DIFadd{Four
Fundamental Forces}} \DIFadd{lecture trajectory (blue line), whereas the LDA embeddings
for }\textit{\DIFadd{Birth of Stars}} \DIFadd{questions (green dots) appear closer to the
}\textit{\DIFadd{Birth of Stars}} \DIFadd{lecture trajectory (green line). The BERT embeddings of
the lectures and questions do not show this property (Supp. Fig.~\ldaVsBERT).
We also examined per-question ``content matches'' between individual questions
and individual moments of each lecture
(Figs.~\ref{fig:question-correlations},~\ldaVsBERT). The timeseries plot of
...
between the questions, whereas BERT is not. The timeseries plots of individual
...
questions about the }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{material are highly
correlated with the embeddings of the }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{lecture,
but not with the embeddings of }\textit{\DIFadd{Birth of Stars}}\DIFadd{, questions about
}\textit{\DIFadd{Birth of Stars}}\DIFadd{, or general physics knowledge questions. We see a
similar pattern with the LDA embeddings of the }\textit{\DIFadd{Birth of Stars}}
\DIFadd{questions (Fig.~\ref{fig:topics}, Supp. Fig.~\topicWeights). In contrast, the
...
\DIFadd{Another application for large language models that does }\textit{\DIFadd{not}} \DIFadd{require
...
models like ChatGPT~\mbox{%DIFAUXCMD
\citep{ChatGPT} }\hskip0pt%DIFAUXCMD
and LLaMa~\mbox{%DIFAUXCMD
\citep{TouvEtal23} }\hskip0pt%DIFAUXCMD
are already
being used to build a new generation of interactive tutoring
systems~\mbox{%DIFAUXCMD
\citep[e.g.,][]{MannEtal23b}}\hskip0pt%DIFAUXCMD
.  Unlike the approach we have taken here,
these systems do not explicitly model what learners know, or how their knowledge 
changes over time with training.  One could imagine building a hybrid system
that combines the best of both worlds: a large language model that can
}\textit{\DIFadd{generate}} \DIFadd{text, combined with a smaller model that can }\textit{\DIFadd{infer}}
\DIFadd{what learners know and how their knowledge changes over time.  Such a hybrid
system could potentially be used to build a new generation of interactive
tutoring systems that are able to adapt to learners' needs in real time, and
that are able to provide more nuanced feedback about what learners know and
what they do not know.
}

\DIFadd{At the opposite end of the spectrum from large language models, one could also
imagine }\textit{\DIFadd{simplifying}} \DIFadd{some aspects of our LDA-based approach by
...
approach. For example, whereas the LDA-based approach often yields a clear peak
in the timeseries of correlations between each question and the corresponding
...
example, one of the example questions examined in Supplementary Figure~\jaccard
asks ``Which of the following occurs as a cloud of atoms gets more dense?''.
The LDA-based matches identify lecture timepoints where the releveant
}\textit{\DIFadd{topics}} \DIFadd{are discussed (e.g., when words like ``cloud,'' ``atom,''
``dense,'' etc., are mentioned }\textit{\DIFadd{together}}\DIFadd{). The Jaccard similarity-based
matches, on the other hand, are strong when }\textit{\DIFadd{any}} \DIFadd{of these words are
mentioned, even if they do not occur together.
}

\DIFadd{We view our approach as occupying a sort of ``sweet spot,'' between much larger
...
\DIFadd{One limitation of our approach is that topic models contain no explicit
...
duration: 7 minutes and 57 seconds). \DIFaddbegin \DIFadd{All participants viewed the videos in the
same order (i.e., }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{followed by }\textit{\DIFadd{Birth of
Stars}}\DIFadd{). While we are not aware of any specific confounds of viewing order, nor
...
\DIFaddend We then hand-created 39 multiple-choice questions: 15 about the conceptual
...
(covering material that was not presented in either video). \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{One of our group's
...
\DIFadd{We note that estimating the specific ``amount'' of conceptual understanding
...
question at that particular moment. The }\DIFaddend full set of questions and answer
...
that (a) each quiz contain exactly 5 questions about Lecture~1, 5 questions
...
Protection of Human Subjects at Dartmouth College. We used this experiment to
...
and last lines. \DIFaddbegin \DIFadd{This $w$ parameter was chosen to match the same number of words
...
work~\mbox{%DIFAUXCMD
\citep{HeusEtal21} }\hskip0pt%DIFAUXCMD
(i.e., 185 words per sliding window).
}


\DIFaddend These sliding windows ramped up and down in length at the beginning and end of
...
in the same number ($w$) of sliding windows. \DIFdelbegin \DIFdel{After performing various }\DIFdelend \DIFaddbegin \DIFadd{We next performed a series of
}\DIFaddend standard text preprocessing \DIFdelbegin \DIFdel{(e.g., }\DIFdelend \DIFaddbegin \DIFadd{steps: }\DIFaddend normalizing case, lemmatizing, removing
punctuation and \DIFaddbegin \DIFadd{removing }\DIFaddend stop-words\DIFdelbegin \DIFdel{), we }\DIFdelend \DIFaddbegin \DIFadd{. We constructed our corpus of stop words by
augmenting the Natural Language Toolkit~\mbox{%DIFAUXCMD
\citep[NLTK; ]{BirdEtal09} }\hskip0pt%DIFAUXCMD
English stop
word list with the following additional words, selected using the stop word
approach suggested by~\mbox{%DIFAUXCMD
\citep{BoydMimn14}}\hskip0pt%DIFAUXCMD
: ``actual,'' ``actually,'' ``also,''
...
remaining words, and lasting for an average of 62.22~seconds. We }\DIFaddend treated the
...
correlations) \DIFdelbegin \DIFdel{insures }\DIFdelend \DIFaddbegin \DIFadd{ensures }\DIFaddend that every question contributes some non-negative amount
to the knowledge estimate\DIFaddbegin \DIFadd{.
}

\subsubsection*{\DIFadd{Estimating the ``smoothness'' of knowledge}}\label{subsec:smoothness}
...
with a radius of 0), where each successive circle had a radius of 0.01
...
participant. The per-radius proportion correct values were then averaged across
both categories of ``reference questions'': those answered correctly and those
answered incorrectly. This yielded two distinct proportion-correct values for
each binned distance (radius) for a specific participant and quiz. From these,
we established the average proportion correct within each radius for both
...
average proportion correct computed earlier. (A parallel analysis was conducted
for the incorrectly answered reference questions.) We display the resulting
distance estimates, for each quiz and reference question status, in
Figure~\ref{fig:smoothness}B}\DIFaddend .
...
Projection (UMAP; \citealp{McInEtal18a, McInEtal18b}) to construct a 2D
