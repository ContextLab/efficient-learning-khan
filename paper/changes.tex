...
%DIF ADD main.tex   Tue Feb 20 22:45:28 2024
%DIF 2a2
...
how they are used~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Mann20, Mann21a}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Mann23b, Mann21a}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
provides a computational framework through which those \DIFdelbegin \DIFdel{document'
s
deeper conceptual meaning }\DIFdelend \DIFaddbegin \DIFadd{documents'
deeper conceptual meanings }\DIFaddend may be quantified, explored, and understood.
...
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Predicting success on held-out questions using estimated
    ...
    questions }\DIFdelendFL about one lecture \DIFdelbeginFL \DIFdelFL{to predict knowledge at }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{based on all other questions
    (from }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{embedding
    coordinate of a held-out question }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same participant and quiz) }\DIFaddendFL about the \DIFdelbeginFL \textit{\DIFdelFL{other}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{same}} \DIFaddendFL lecture
    \DIFdelbeginFL \DIFdelFL{. In the
    bottom row }\DIFdelendFL (``Within-lecture''\DIFaddbeginFL \DIFaddFL{; middle rows}\DIFaddendFL ), \DIFdelbeginFL \DIFdelFL{we used all but one }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and knowledge for each }\DIFaddendFL question about
    one lecture \DIFdelbeginFL \DIFdelFL{to predict knowledge at }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{based on all questions (from }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{embedding coordinate of a held-out
    question }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same participant and quiz)
    }\DIFaddendFL about the \DIFdelbeginFL \textit{\DIFdelFL{same}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{other}} \DIFaddendFL lecture \DIFaddbeginFL \DIFaddFL{(``Across-lecture''; bottom rows)}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{We repeated each of these
    analyses using all possible held-out questions for each quiz and
    participant. }\DIFdelendFL The
    \DIFdelbeginFL \DIFdelFL{arrows at the tops of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{backgrounds in }\DIFaddendFL each panel \DIFdelbeginFL \DIFdelFL{indicate whether }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{display kernel density estimates of }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{average predicted knowledge was higher for held-out }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{relative
    observed proportions of }\DIFaddendFL correctly \DIFdelbeginFL \DIFdelFL{answered
    }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{left}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{blue}\DIFaddendFL ) \DIFdelbeginFL \DIFdelFL{or }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{versus }\DIFaddendFL incorrectly \DIFdelbeginFL \DIFdelFL{answered }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{right}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{red}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{answered
    }\DIFaddendFL questions\DIFaddbeginFL \DIFaddFL{, for each level of estimated knowledge along the $x$-axis}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{The
    ...
    ribbons denote 95\% confidence intervals.}\DIFaddendFL }
...
We repeated each of these analysesusing all possible held-out questionsfor each quiz and participant .
}\DIFdelend \DIFaddbegin \DIFadd{predictions
held across the content areas of the two lectures.
}\DIFaddend 

\DIFdelbegin \DIFdel{For the initial quizzes participants took (prior to watching either lecture), predicted knowledge tended to be low overall, and relatively
unstructured (Fig.~\ref{fig:predictions}, left column). When we held out
individual questions and predicted their knowledge at the held-out questions' embedding coordinates, we found no reliable differences in the predictions when
the }\DIFdelend \DIFaddbegin \DIFadd{In performing these analyses, our null hypothesis is that the knowledge
...
single }\textit{\DIFadd{incorrectly}} \DIFadd{answered question, the proportion of remaining
questions answered correctly would be $\frac{n}{q - 1}$. In this way, the
proportion of correctly answered remaining questions is always }\textit{\DIFadd{lower}}
\DIFadd{when the }\DIFaddend held-out \DIFdelbegin \DIFdel{question had been correctly versus incorrectly answered. This
...
the spatial relationships among the quiz questions' embeddings }\textit{\DIFadd{are}}
\DIFadd{predictive of participants' knowledge about the questions' content, then we
would expect }\textit{\DIFadd{higher}} \DIFadd{estimated knowledge for }\DIFaddend held-out \DIFdelbegin \textit{\DIFdel{Four Fundamental Forces Questions}} %DIFAUXCMD
...
questions from one lecture to predict }\DIFdelend \DIFaddbegin \DIFadd{correctly (versus
...
with our null hypothesis: higher estimated knowledge }\DIFaddend at the embedding
...
\DIFdel{: $\U = 7681.5,~p = 0.746$; }\textit{\DIFdel{Birth of
Stars}}%DIFAUXCMD
\DIFdel{: $\U = 8125,~p = 0.204$). We believe that this
...
\DIFdelbegin \DIFdel{After watching }\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdel{, predicted knowledge for
}\DIFdelend \DIFaddbegin \DIFadd{We observed a similar pattern of results when used this approach to estimate
participants' knowledge about }\DIFaddend held-out \DIFdelbegin \DIFdel{questions that were answered correctly (from the second quiz;
...
we predicted knowledge
across-lectures (}\textit{\DIFdel{Four Fundamental Forces}}%DIFAUXCMD
\DIFdel{: $\U =
6749.5,~p = 0.014$; }\textit{\DIFdel{Birth of Stars}}%DIFAUXCMD
\DIFdel{: $\U = 8480,~p = 0.016$), and when
we predicted knowledge at the }\DIFdelend \DIFaddbegin \DIFadd{questions from one lecture using their
performance on other questions from the }\textit{\DIFadd{same}} \DIFadd{lecture. Specifically,
for Quiz~1 questions (i.e., prior to watching either), participants' estimated
knowledge for the }\DIFaddend embedding coordinates of held-out \textit{Four Fundamental
Forces}\DIFdelbegin \DIFdel{questions }\DIFdelend \DIFaddbegin \DIFadd{-related questions estimated }\DIFaddend using other \textit{Four Fundamental
Forces}\DIFdelbegin \DIFdel{questions from the same quiz and participant ($\U = 7224,~p < 0.001$). This
...
some degree of random guessing on Quiz~1. When we repeated these }\DIFaddend within-lecture
\DIFdelbegin \DIFdel{knowledge predictions at
knowledge at embedding space coordinates of }\DIFdelend \DIFaddbegin \DIFadd{analyses using questions from Quiz~2 (which participants took immediately after
viewing }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{but prior to viewing }\DIFaddend \textit{Birth of
...
%DIFDELCMD < %%%
\DIFdel{Finally, after
watching }\textit{\DIFdel{Birth of Stars}}%DIFAUXCMD
\DIFdel{, predicted knowledge for
...
incorrectly answered questions . This held when we included all questions in the analysis ($\U = 38279,~p = 0.022$), when we carried out across-lecture
predictions (}\DIFdelend \DIFaddbegin \DIFadd{content
(e.g., prior to having watched it), whereas they might have been drawing on
some structured knowledge about the }\DIFaddend \textit{Four Fundamental Forces} \DIFdelbegin \DIFdel{: $\U = 6684.5,~p = 0.032$;
}\DIFdelend \DIFaddbegin \DIFadd{content
(e.g., from having just watched it). When we applied this approach to Quiz~3
responses (given immediately after viewing }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{: $\U = 6414.5,~p = 0.002$), and and when we carried out
}\DIFdelend \DIFaddbegin \DIFadd{), we found
that }\DIFaddend within-lecture knowledge \DIFdelbegin \DIFdel{predictions for held-out }\DIFdelend \DIFaddbegin \DIFadd{estimates for }\DIFaddend \textit{Birth of Stars}\DIFdelbegin \DIFdel{questions using other }\textit{\DIFdel{Birth of Stars}} %DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{estimates for }\DIFaddend \textit{Four Fundamental
Forces} questions \DIFdelbegin \DIFdel{using other }\textit{\DIFdel{Four Fundamental
Forces}} %DIFAUXCMD
\DIFdel{questions from the
same quiz and participant ($\U = 6734,~p = 0.027$). Specifically, on Quiz }\DIFdelend \DIFaddbegin \DIFadd{answered on Quiz~}\DIFaddend 3 \DIFdelbegin \DIFdel{, our knowledge predictions for held-out correctly
answered questions about }\textit{\DIFdel{Four Fundamental Forces}} %DIFAUXCMD
\DIFdel{were reliably
}\textit{\DIFdel{lower}} %DIFAUXCMD
\DIFdel{than those for their incorrectly answered counterparts.
}\DIFdelend \DIFaddbegin \DIFadd{were no longer directly related to the
...
quiz)}\DIFaddend . If this forgetting happens in a relatively ``random'' way (with respect
to spatial distance within the \DIFdelbegin \DIFdel{text
}\DIFdelend embedding space), then it could explain why some
...
participants answered incorrectly. Taken together, \DIFdelbegin \DIFdel{the results
in Figure~\ref{fig:predictions} indicate }\DIFdelend \DIFaddbegin \DIFadd{these within-lecture results
suggest }\DIFaddend that our approach can \DIFdelbegin \DIFdel{reliably predict acquired knowledge (especially about recently
learnedcontent) , and }\DIFdelend \DIFaddbegin \DIFadd{distinguish between questions about different
...
\DIFadd{Finally, we used this approach to estimate participants' knowledge about
held-out questions from one lecture using their performance on questions from
the }\textit{\DIFadd{other}} \DIFadd{lecture. Here we again observed a similar pattern of
...
\citet{BoydEtal14}}\hskip0pt%DIFAUXCMD
...
work~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{HeusEtal21} }\hskip0pt%DIFAUXCMD
(i.e., 185 words per sliding window)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[][i.e., 185 words per sliding window]{HeusEtal21}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
approaches suggested by~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{BoydEtal14}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{BoydEtal14}}\hskip0pt%DIFAUXCMD
...
\DIFadd{In the set of analyses reported in Figure~\ref{fig:predictions}, we assessed whether estimates of participants' knowledge at the embedding coordinates of individual quiz questions could reliably predict their ability to correctly answer those questions.
In essence, we treated each question a given participant answered on a given quiz as a ``lecture'' consisting of a single timepoint, and used Equation~\ref{eqn:prop} to estimate the participant's knowledge for its embedding coordinate based on their performance on all }\textit{\DIFadd{other}} \DIFadd{questions they answered on that same quiz (``All questions''; Fig.~\ref{fig:predictions}, top row). 
...
%DIF > We also computed analogous knowledge estimates for each lecture-related question based on (1) all other questions answered by the same participant, on the same quiz about the same lecture as the target question (``Within-lecture''; Fig.~\ref{fig:predictions}, middle rows) and (2) all questions from the same participant and quiz about the other of the two lectures (``Across-lecture''; Fig.~\ref{fig:predictions}, bottom rows).
}

\DIFadd{In each version of this analysis (i.e., row in Fig.~\ref{fig:predictions}), and separately for each of the three quizzes (i.e., column in Fig.~\ref{fig:predictions}),  we then fit a generalized linear mixed model (GLMM) with a logistic link function to the set of knowledge estimates for all questions that participants answered on the given quiz.
We implemented these models in }\texttt{\DIFadd{R}} \DIFadd{using the }\texttt{\DIFadd{lme4}} \DIFadd{package~\mbox{%DIFAUXCMD
\citep{BateEtal15a} }\hskip0pt%DIFAUXCMD
and fit them following guidance from~\mbox{%DIFAUXCMD
\citet{BateEtal15b} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{MatuEtal17}}\hskip0pt%DIFAUXCMD
. Specifically, we initially fit each model with the maximal random effects structure afforded our data, which we identified as:
}\[
    \DIFadd{\mathtt{accuracy \sim knowledge + (knowledge\ \vert\ participant) + (knowledge\ \vert\ question)}
}\]
\DIFadd{Here, ``}\texttt{\DIFadd{accuracy}}\DIFadd{'' is a binary value indicating whether each target question was answered correctly or incorrectly, ``}\texttt{\DIFadd{knowledge}}\DIFadd{'' is estimated knowledge at each target question's embedding coordinate, ``}\texttt{\DIFadd{participant}}\DIFadd{'' is a unique identifier assigned to each participant, and ``}\texttt{\DIFadd{question}}\DIFadd{'' is a unique identifier assigned to each quiz question.
For models we fit using knowledge estimates for target questions about multiple content areas (i.e., in the ``All questions'' version of the analysis), we also included an additional random effect term, $\mathtt{(knowledge\ \vert\ lecture)}$, where ``}\texttt{\DIFadd{lecture}}\DIFadd{'' is a categorical value denoting whether the target question was about }\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{, }\textit{\DIFadd{Birth of Stars}}\DIFadd{, or general physics knowledge.
Note that with our coding scheme, identifiers for each }\texttt{\DIFadd{question}} \DIFadd{are implicitly nested within levels of }\texttt{\DIFadd{lecture}} \DIFadd{and do not require explicit nesting in our model formula.
}

\DIFadd{When necessary, we then iteratively removed random effects from the maximal model until it successfully converged with a full rank (i.e., non-singular) random effects variance-covariance matrix. 
...
However, in order to ensure we fit all of the models for these analyses in a consistent way, we devised some simple heuristics for choosing which boundary-estimated parameter to eliminate in each stepwise reduction of the model's complexity, based on a combination of popular approaches~\mbox{%DIFAUXCMD
\citep{BarrEtal13,BateEtal15b,MatuEtal17}}\hskip0pt%DIFAUXCMD
.
...
%DIF > Additionally, if constraining a boundary-estimated correlation term to 0 allowed the model to converge with a full rank covariance matrix, we left the correlation term constrained rather than removing the associated slope and/or intercept component.
Third, when choosing which of two boundary-estimated variance components of the }\textit{\DIFadd{same}} \DIFadd{order to exclude from the model first, we took one of two approaches.
...
Finally, once the model successfully converged with a full rank covariance matrix, we did not remove random effects for any other reasons (e.g., to lowered AIC, increase log-likelihood, etc.). 
We chose this stopping criterion as a conceptual ``middle ground'' between two popular but opposing approaches to model selection that advocate for either retaining the maximal model that allows convergence, regardless of singular fits~\mbox{%DIFAUXCMD
\citep[at the potential cost of decreased power; e.g.,~][]{BarrEtal13} }\hskip0pt%DIFAUXCMD
or performing additional hypothesis tests on individual parameters and removing non-significant components to achieve a parsimonious model~\mbox{%DIFAUXCMD
\citep[at the potential cost of increased Type I error rates; e.g.,~][]{BateEtal15b}}\hskip0pt%DIFAUXCMD
.
...
\DIFadd{To assess the predictive value of our knowledge estimates for individual quiz questions, we used the same sets of observations used to fit each GLMM to fit a second set of ``null'' models (similarly with a logistic link function). We fit these models with the formula:
}\[
    \DIFadd{\mathtt{accuracy \sim (1\ \vert\ participant) + (1\ \vert\ question)}
}\]
...
%DIF > $$
%DIF > \begin{aligned}
%DIF >   \operatorname{accuracy}_{i}  &\sim \operatorname{Binomial}(n = 1, \operatorname{prob}_{\operatorname{accuracy} = \operatorname{correct}} = \widehat{P}) \\
%DIF >     \log\left[\frac{\hat{P}}{1 - \hat{P}} \right] &=\alpha_{j[i],k[i]} \\
%DIF > \left(
%DIF >   \begin{array}{c}
%DIF >     \begin{aligned}
%DIF >       &\alpha_{j} \\
...
%DIF >   \begin{array}{c}
%DIF >     \begin{aligned}
%DIF >       &\gamma_{0}^{\alpha} + \gamma_{1k[i]}^{\alpha}(\operatorname{knowledge}) \\
%DIF >       &\mu_{\gamma_{1j}}
%DIF >     \end{aligned}
%DIF >   \end{array}
%DIF > \right)
...
%DIF > \left(
%DIF >   \begin{array}{cc}
%DIF >      \sigma^2_{\alpha_{j}} & \rho_{\alpha_{j}\gamma_{1j}} \\
%DIF >      \rho_{\gamma_{1j}\alpha_{j}} & \sigma^2_{\gamma_{1j}}
%DIF >   \end{array}
%DIF > \right)
%DIF >  \right)
%DIF >     \text{, for participant j = 1,} \dots \text{,J} \\
%DIF > \left(
%DIF >   \begin{array}{c}
%DIF >     \begin{aligned}
%DIF >       &\alpha_{k} \\
...
%DIF > \left(
%DIF >   \begin{array}{cc}
%DIF >      \sigma^2_{\alpha_{k}} & \rho_{\alpha_{k}\gamma_{1k}} \\
%DIF >      \rho_{\gamma_{1k}\alpha_{k}} & \sigma^2_{\gamma_{1k}}
%DIF >   \end{array}
%DIF > \right)
%DIF >  \right)
%DIF >     \text{, for question k = 1,} \dots \text{,K}
%DIF > \end{aligned}
%DIF > $$
