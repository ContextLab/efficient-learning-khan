...
%DIF DEL old.tex    Tue Oct 24 09:44:50 2023
%DIF ADD main.tex   Wed Oct 25 00:02:03 2023
...
%DIF -------
\newcommand{\forcesCorrs}{3} %DIF > 
\newcommand{\bosCorrs}{4} %DIF > 
\newcommand{\jaccard}{5} %DIF > 
\newcommand{\ldaVsBERT}{6} %DIF > 
\newcommand{\individualKnowledgeMapsA}{7} %DIF > 
\newcommand{\individualKnowledgeMapsB}{8} %DIF > 
\newcommand{\individualKnowledgeMapsC}{9} %DIF > 
\newcommand{\individualLearningMapsA}{10} %DIF > 
\newcommand{\individualLearningMapsB}{11} %DIF > 
 %DIF > 
\newcommand{\U}{{\fontfamily{serif}\selectfont\ensuremath{\mathrm{U}}}} %DIF > 
 %DIF > 
%DIF -------
...
neuroscience\DIFdelbegin \DIFdel{(e.g.,~\mbox{%DIFAUXCMD
\citealp{Kato40, Gall00, ScotEtal07, HallGree08, Macl05}}\hskip0pt%DIFAUXCMD
)}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[e.g.,][]{Kato40, Gall00, ScotEtal07, HallGree08,
  Macl05}}\hskip0pt%DIFAUXCMD
}\DIFaddend , has profound analogs in the fields of natural language
...
models\DIFdelbegin \DIFdel{(e.g.,~\mbox{%DIFAUXCMD
\citealp{LandDuma97, DeerEtal90, BleiEtal03,
BleiLaff06, MikoEtal13a, CerEtal18, BrowEtal20}}\hskip0pt%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[e.g.,][]{LandDuma97, DeerEtal90, BleiEtal03,
  BleiLaff06, MikoEtal13a, CerEtal18, BrowEtal20, ViswEtal17} }\hskip0pt%DIFAUXCMD
}\DIFaddend also
...
elements within and across documents, but \DIFaddbegin \DIFadd{(in many cases) }\DIFaddend also
...
\DIFaddbegin \DIFadd{To be clear, this is not to say that text embedding models themselves
...
}\textit{\DIFadd{structure}} \DIFadd{of text documents beyond their surface-level contents
...
}\DIFaddend According to these models,
...
feature vectors to two conceptually related documents, \textit{even
  when the specific words contained in those documents have \DIFdelbegin \DIFdel{very little
}\DIFdelend \DIFaddbegin \DIFadd{limited
  }\DIFaddend overlap}. \DIFaddbegin \DIFadd{In this way, ``concepts'' are defined implicitly by the
model's geometry~\mbox{%DIFAUXCMD
\citep[e.g., how the embedding coordinate of a given
...
presented in each moment of each lecture, we can obtain a detailed \DIFdelbegin \DIFdel{timecourse
}\DIFdelend \DIFaddbegin \DIFadd{time course
}\DIFaddend describing how much ``knowledge'' the participant has about \DIFaddbegin \DIFadd{the content
presented at }\DIFaddend any part of the lecture. As shown in
...
Equation~\ref{eqn:prop} to \DIFdelbegin \DIFdel{estimate }\DIFdelend \DIFaddbegin \DIFadd{predict }\DIFaddend each participant's knowledge at the given
...
\DIFdelbegin \DIFdel{estimates }\DIFdelend \DIFaddbegin \DIFadd{predicted knowledge values }\DIFaddend into two distributions: one for the \DIFdelbegin \DIFdel{estimated }\DIFdelend \DIFaddbegin \DIFadd{predicted }\DIFaddend knowledge at the
coordinates of \textit{correctly} answered questions, and another for the
\DIFdelbegin \DIFdel{estimated }\DIFdelend \DIFaddbegin \DIFadd{predicted }\DIFaddend knowledge at the coordinates of \textit{incorrectly} answered
...
compare the means of these distributions of \DIFdelbegin \DIFdel{estimated }\DIFdelend \DIFaddbegin \DIFadd{predicted }\DIFaddend knowledge.
...
\DIFaddendFL 
    ...
    \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Estimating knowledge at the embedding coordinates of
    held-out questions.}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Predicting knowledge at the embedding coordinates of
    held-out questions.}} \DIFaddendFL Separately for each quiz (\DIFdelbeginFL \DIFdelFL{panel}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{column}\DIFaddendFL ), we plot the
    ...
    \DIFdelbeginFL \DIFdelFL{$t$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Mann-Whitney $\U$}\DIFaddendFL -tests reported in each panel are between the
    distributions of \DIFdelbeginFL \DIFdelFL{estimated
    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{predicted }\DIFaddendFL knowledge at the coordinates of correctly \DIFdelbeginFL \DIFdelFL{versus }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and
    }\DIFaddendFL incorrectly answered held-out questions. \DIFaddbeginFL \DIFaddFL{In the top row (``All
    ...
    coordinate of a held-out question about the }\textit{\DIFaddFL{other}} \DIFaddFL{lecture. In the
    ...
    question about the }\textit{\DIFaddFL{same}} \DIFaddFL{lecture. We repeated each of these
    ...
    (left) or incorrectly answered (right) questions.}\DIFaddendFL }

    \label{fig:predictions}
\end{figure}

\DIFaddbegin \DIFadd{We carried out these analyses in three different ways. First, we used all (but one) of
...
question about the }\textit{\DIFadd{other}} \DIFadd{lecture, from the same quiz and participant
(``Across-lecture'' in Fig.~\ref{fig:predictions}). This test was intended to
test the }\textit{\DIFadd{generalizability}} \DIFadd{of our approach by asking whether our
...
coordinate of a held-out question about the }\textit{\DIFadd{same}} \DIFadd{lecture, from the same
quiz and participant (``Within-lecture'' in Fig.~\ref{fig:predictions}). This
test was intended to test the }\textit{\DIFadd{specificity}} \DIFadd{of our approach by asking
...
\DIFaddend For the initial quizzes participants took (prior to watching either lecture),
\DIFdelbegin \DIFdel{participants' estimated }\DIFdelend \DIFaddbegin \DIFadd{predicted }\DIFaddend knowledge tended to be low overall, and relatively
...
individual questions and \DIFdelbegin \DIFdel{estimated }\DIFdelend \DIFaddbegin \DIFadd{predicted }\DIFaddend their knowledge at the held-out questions'
embedding coordinates, we found no reliable differences in the \DIFdelbegin \DIFdel{estimates }\DIFdelend \DIFaddbegin \DIFadd{predictions }\DIFaddend when
...
\DIFdelbegin \DIFdel{After watching the first lecture, estimated }\DIFdelend \DIFaddbegin 

\DIFadd{After watching }\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{, predicted }\DIFaddend knowledge for
held-out \DIFdelbegin \DIFdel{correctly answered questions }\DIFdelend \DIFaddbegin \DIFadd{questions that were answered correctly }\DIFaddend (from the second quiz;
Fig.~\ref{fig:predictions}, middle \DIFdelbegin \DIFdel{panel}\DIFdelend \DIFaddbegin \DIFadd{column}\DIFaddend ) exhibited a \DIFaddbegin \DIFadd{significant }\DIFaddend positive
shift relative to held-out \DIFdelbegin \DIFdel{incorrectly answered questions }\DIFdelend \DIFaddbegin \DIFadd{questions that were answered incorrectly. This held
when we included all questions in the analysis ($\U = 58332,~p < 0.001$), when
we predicted knowledge across-lectures }\DIFaddend (\DIFdelbegin \DIFdel{$t(633) = 3.961,~p < 0.001$). This
second quiz provides the maximally sensitive test for our knowledge predictions , since (if knowledge is estimated accurately)participants' Quiz~2
responses should demonstrate specific knowledge
about Lecture~1 content , but
knowledge about Lecture~2 and general physics concepts should be roughly
unchanged from before they watched Lecture~1. After watching the second lecture, estimated knowledge }\DIFdelend \DIFaddbegin \textit{\DIFadd{Four Fundamental Forces}}\DIFadd{: $\U =
6749.5,~p = 0.014$; }\textit{\DIFadd{Birth of Stars}}\DIFadd{: $\U = 8480,~p = 0.016$), and when
we predicted knowledge at the embedding coordinates of held-out }\textit{\DIFadd{Four
Fundamental Forces}} \DIFadd{questions using other }\textit{\DIFadd{Four Fundamental Forces}}
\DIFadd{questions from the same quiz and participant ($\U = 7224,~p < 0.001$). This
difference did }\textit{\DIFadd{not}} \DIFadd{hold for within-lecture knowledge predictions at
knowledge at embedding space coordinates of }\textit{\DIFadd{Birth of Stars}} \DIFadd{questions
...
about the content of the }\textit{\DIFadd{Birth of Stars}} \DIFadd{material is relatively low
everywhere in that region of text embedding space.
}

\DIFadd{Finally, after watching }\textit{\DIFadd{Birth of Stars}}\DIFadd{, predicted knowledge for
held-out correctly answered questions }\DIFaddend (from the third quiz;
Fig.~\ref{fig:predictions}, right \DIFdelbegin \DIFdel{panel) }\DIFdelend \DIFaddbegin \DIFadd{column) was higher than for held-out
...
predictions (}\textit{\DIFadd{Four Fundamental Forces}}\DIFadd{: $\U = 6684.5,~p = 0.032$;
}\textit{\DIFadd{Birth of Stars}}\DIFadd{: $\U = 6414.5,~p = 0.002$), and and when we carried out
within-lecture knowledge predictions for held-out }\textit{\DIFadd{Birth of Stars}}
\DIFadd{questions using other }\textit{\DIFadd{Birth of Stars}} \DIFadd{questions from the same quiz and
participant ($\U = 6126,~p = 0.006$). However, we found the }\textit{\DIFadd{opposite}}
\DIFadd{effect when we carried out within-lecture knowledge predictions }\DIFaddend for \DIFdelbegin \textit{\DIFdel{all}} %DIFAUXCMD
\DIFdel{questionsexhibited a positive shift. However, the
estimated knowledge for }\DIFdelend held-out
\DIFaddbegin \textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions using other }\textit{\DIFadd{Four Fundamental
Forces}} \DIFadd{questions from the same quiz and participant ($\U = 6734,~p = 0.027$).
Specifically, on Quiz 3, our knowledge predictions for held-out }\DIFaddend correctly
answered questions \DIFdelbegin \DIFdel{remained
greater than that for held-out incorrectly answered questions ($t(628) = 2.045,~p
= 0.041$) }\DIFdelend \DIFaddbegin \DIFadd{about }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{were reliably
}\textit{\DIFadd{lower}} \DIFadd{than those for their incorrectly answered counterparts.
Speculatively, we suggest that this may reflect participants forgetting some of
the }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{content. If this forgetting happens in a
...
}\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{were answered incorrectly, even if questions
...
about ``forgotten'' knowledge that participants answered incorrectly}\DIFaddend . \DIFdelbegin \DIFdel{This third contrast reflects a ceiling effect: when knowledge is
relatively high everywhere, the signal differentiating what is known versus
unknown is relatively weak. }\DIFdelend Taken
together, \DIFdelbegin \DIFdel{this set of analyses demonstrates
that our
knowledge prediction framework is most informative when participants exhibit variability in their knowledge of the content captured by the
text
embedding model}\DIFdelend \DIFaddbegin \DIFadd{the results in Figure~\ref{fig:predictions} indicate that our
...
\DIFadd{That the knowledge predictions derived from the text embedding space reliably
...
question }\textit{\DIFadd{in}}\DIFadd{correctly. Again, as we move farther away from $x$ in the
embedding space, how does the likelihood that the participant does }\textit{\DIFadd{not}}
\DIFadd{know about a coordinate's content change with distance? We reasoned that,
...
the question embedded }\textit{\DIFadd{at}} \DIFadd{$x$. Whereas at another extreme, once we
...
}\textit{\DIFadd{overall}} \DIFadd{proportion of correctly answered questions. In other words,
...
\begin{figure}[t]
    ...
    \caption{\textbf{\DIFaddFL{Knowledge falls off gradually in text embedding space.}}
    \textbf{\DIFaddFL{A. Performance versus distance.}} \DIFaddFL{For each participant, for each
    ...
    the average proportion correct across }\textit{\DIFaddFL{all}} \DIFaddFL{questions included in
    the analysis at the given distance. }\textbf{\DIFaddFL{B. Maximum distance for which
    performance is reliably different from the average.}} \DIFaddFL{We used a bootstrap
    procedure (see~}\nameref{subsec:smoothness}\DIFaddFL{) to estimate the point at which
    ...
    questions from each quiz. }\textbf{\DIFaddFL{All panels.}} \DIFaddFL{Error ribbons denote
...
sliding windows with \DIFdelbegin \DIFdel{(up to)~}\DIFdelend $k = 100$~topics. Conceptually, increasing the
...
contained in the two lectures. \DIFaddbegin \DIFadd{We note that we used these 2D maps solely for
...
using the 15-topic model. }\DIFaddend Aside from increasing the number of topics from 15 to
...
\DIFdelbegin \DIFdel{Over the }\DIFdelend \DIFaddbegin \DIFadd{We view our work as making several contributions to the study of how people
...
}\textit{\DIFadd{conceptualize}} \DIFadd{similarly the content reflected by nearby embedding
...
(e.g., the domain of physics lectures and questions) }\textit{\DIFadd{and}} \DIFadd{sufficiently
...
LDA embeddings for }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{questions (blue dots)
appear closer to the }\textit{\DIFadd{Four Fundamental Forces}} \DIFadd{lecture trajectory (blue
line), whereas the LDA embeddings for }\textit{\DIFadd{Birth of Stars}} \DIFadd{questions (green
dots) appear closer to the }\textit{\DIFadd{Birth of Stars}} \DIFadd{lecture trajectory (green
...
language model that can }\textit{\DIFadd{generate}} \DIFadd{text, combined with a smaller model
that can }\textit{\DIFadd{infer}} \DIFadd{what learners know and how their knowledge changes over
...
relevant }\textit{\DIFadd{topics}} \DIFadd{are discussed (e.g., when words like ``cloud,''
``atom,'' ``dense,'' etc., are mentioned }\textit{\DIFadd{together}}\DIFadd{). The Jaccard
similarity-based matches, on the other hand, are strong when }\textit{\DIFadd{any}} \DIFadd{of
...
\DIFadd{Within the }\DIFaddend past several years, the global pandemic \DIFdelbegin \DIFdel{has }\DIFdelend forced many educators to
...
across people? How might we lower barriers \DIFdelbegin \DIFdel{of access
to }\DIFdelend \DIFaddbegin \DIFadd{to receiving }\DIFaddend a high-quality
education?
...
and/or other measurements\DIFdelbegin \DIFdel{(e.g., neurophysiological data;~\mbox{%DIFAUXCMD
\citealp{NguyEtal22,
MeshEtal20, PoulEtal17, BeviEtal19, DikkEtal17}}\hskip0pt%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[e.g., neurophysiological data; ][]{NguyEtal22,
MeshEtal20, PoulEtal17, BeviEtal19, DikkEtal17} }\hskip0pt%DIFAUXCMD
}\DIFaddend might help to inform these
...
(covering material that was not presented in either video). \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{To help broaden the
...
the question at that particular moment. The }\DIFaddend full set of questions and answer
choices may be found in Supplementary Table~\questions.  \DIFaddbegin \DIFadd{The final set of
...
that (a) each quiz \DIFdelbegin \DIFdel{contain }\DIFdelend \DIFaddbegin \DIFadd{contained }\DIFaddend exactly 5 questions about Lecture~1, 5 questions
...
randomized. \DIFdelbegin \DIFdel{Our }\DIFdelend \DIFaddbegin \DIFadd{We obtained informed consent from all participants, and our
}\DIFaddend experimental protocol was approved by the Committee for the Protection of Human
...
\DIFaddbegin \subsubsection*{\DIFadd{Statistics}}

\DIFadd{All of the statistical tests performed in our study were two-sided. The 95\%
...
\DIFaddend \subsubsection*{Constructing text embeddings of multiple lectures and questions}\label{subsec:topic-modeling}
...
representational space. Briefly, our approach uses a topic model\DIFdelbegin \DIFdel{(Latent
Dirichlet Allocation;~\mbox{%DIFAUXCMD
\citealp{BleiEtal03}}\hskip0pt%DIFAUXCMD
), }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[Latent
Dirichlet Allocation; ][]{BleiEtal03} }\hskip0pt%DIFAUXCMD
}\DIFaddend trained on a set of documents, to
discover a set of \DIFdelbegin \DIFdel{(up to) }\DIFdelend $k$ ``topics'' or ``themes.'' Formally, each topic is
defined as a distribution of weights over \DIFdelbegin \DIFdel{each word }\DIFdelend \DIFaddbegin \DIFadd{words }\DIFaddend in the model's vocabulary
...
that are semantically related \DIFdelbegin \DIFdel{or tend }\DIFdelend \DIFaddbegin \DIFadd{(as inferred from their tendency }\DIFaddend to co-occur in
the same \DIFdelbegin \DIFdel{documents}\DIFdelend \DIFaddbegin \DIFadd{document)}\DIFaddend . After fitting a topic model, each document in the training
...
an appropriate $k$ for our model, \DIFaddbegin \DIFadd{as a starting point, }\DIFaddend we identified the
...
topics. \DIFaddbegin \DIFadd{We found that with a limited number of additional adjustments
following~\mbox{%DIFAUXCMD
\citep{BoydEtal14}}\hskip0pt%DIFAUXCMD
, such as removing corpus-specific stop-words, the
model yielded (subjectively) sensible and coherent topics. }\DIFaddend The distribution of
...
transcripts\DIFdelbegin \DIFdel{(which, in prior work, we have found to be of sufficiently near-human
quality to yield reliable data in behavioral studies;~\mbox{%DIFAUXCMD
\citealp{ZimaEtal18}}\hskip0pt%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[which, in prior work, we have found to be of sufficiently
near-human quality to yield reliable data in behavioral
studies; ][]{ZimaEtal18} }\hskip0pt%DIFAUXCMD
}\DIFaddend when developing our framework in order to make
...
approaches suggested by~\mbox{%DIFAUXCMD
\citep{BoydEtal14}}\hskip0pt%DIFAUXCMD
...
resulting \DIFdelbegin \DIFdel{timeseries }\DIFdelend \DIFaddbegin \DIFadd{time series }\DIFaddend to one vector per second. We also used the fitted model to
obtain topic vectors for each question in our pool (see Supp.\DIFaddbegin \DIFadd{~}\DIFaddend Tab.~\questions).
...
Projection\DIFdelbegin \DIFdel{(UMAP; \mbox{%DIFAUXCMD
\citealp{McInEtal18a, McInEtal18b}}\hskip0pt%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep[UMAP; ][]{McInEtal18a, McInEtal18b} }\hskip0pt%DIFAUXCMD
}\DIFaddend to construct a 2D
projection of the text embedding space. \DIFdelbegin \DIFdel{Sampling the original }\DIFdelend \DIFaddbegin \DIFadd{Whereas our main analyses used a
...
sampling this }\DIFaddend 100-dimensional space at high resolution to obtain an adequate
...
Conceptualization: \DIFdelbegin \DIFdel{PCF, ACH, and JRM.Methodology: PCF, ACH, and
JRM.Software:
PCF.Validation: PCF.}\DIFdelend \DIFaddbegin \DIFadd{P.C.F., A.C.H., and J.R.M. Methodology: P.C.F., A.C.H., and
J.R.M. Software: P.C.F. Validation: P.C.F. }\DIFaddend Formal analysis: \DIFdelbegin \DIFdel{PCF.Resources: PCF,
ACH, and JRM.}\DIFdelend \DIFaddbegin \DIFadd{P.C.F. Resources: P.C.F.,
A.C.H., and J.R.M. }\DIFaddend Data curation: \DIFdelbegin \DIFdel{PCF.}\DIFdelend \DIFaddbegin \DIFadd{P.C.F. }\DIFaddend Writing (original draft): \DIFdelbegin \DIFdel{JRM.}\DIFdelend \DIFaddbegin \DIFadd{J.R.M. }\DIFaddend Writing
(review and editing): \DIFdelbegin \DIFdel{PCF, ACH, and JRM.Visualization: PCF and JRM.Supervision: JRM.}\DIFdelend \DIFaddbegin \DIFadd{P.C.F., A.C.H., and J.R.M. Visualization: P.C.F. and J.R.M.
Supervision: J.R.M. }\DIFaddend Project administration: \DIFdelbegin \DIFdel{PCF.}\DIFdelend \DIFaddbegin \DIFadd{P.C.F. }\DIFaddend Funding acquisition: \DIFdelbegin \DIFdel{JRM.}\DIFdelend \DIFaddbegin \DIFadd{J.R.M.
}\DIFaddend 


\section*{Data \DIFdelbegin \DIFdel{and code }\DIFdelend availability}

All of the data analyzed in this manuscript \DIFdelbegin \DIFdel{, along with all }\DIFdelend \DIFaddbegin \DIFadd{may be found at
https://github.com/Con\-text\-Lab/eff\-ic\-ient-learn\-ing-khan.
}

\section*{\DIFadd{Code availability}}

\DIFadd{All }\DIFaddend of the code for running our experiment and carrying out the analyses may be
...
by NSF CAREER Award Number 2145172 to \DIFdelbegin \DIFdel{JRM}\DIFdelend \DIFaddbegin \DIFadd{J.R.M}\DIFaddend . The content is solely the
